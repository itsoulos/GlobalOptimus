...
if [ $MODEL = "mlp" ]; then
  ...
  MODELPARAMS="--opt_method=$METHOD --rbf_nodes=10 --rbf_factor=3.0 --model_trainfile=$DATAPATH/$1.train --model_testfile=$DATAPATH/$1.test"
...
### nnc_enablebound: Enable or disable the usage of bounding techniques during the local search procedure.
  MODELPARAMS="--model_trainfile=$DATAPATH/$1.train --model_testfile=$DATAPATH/$1.test --nnc_popcount=500 --nnc_popsize=500 --nnc_popgens=500 --nnc_popsrate=0.1 --nnc_popmrate=0.05 --nnc_lsearchrate=0.00 --nnc_lsearchmethod=none --nnc_crossitems=10 --nnc_lsearchiters=50 --nnc_lsearchitems=0 --nnc_steadystate=no --nnc_weightfactor=2.0  --nnc_enablebound=no --nnc_pretrain=yes"
fi
...
METHODPARAMS="--population=100 --maxiters=200 --localsearchRate=0.00 --technique=0"
  ...
  METHODPARAMS="--gen_lrate=0.001 --gen_srate=0.1 --gen_mrate=0.05 --gen_tsize=8 --gen_selection=tournament --gen_crossover=double --gen_mutation=double --gen_count=500 --gen_maxiters=200 --gen_lsearchmethod=siman --gen_lsearchgens=20 --gen_lsearchitems=20"
