#############################################################################
# Makefile for building: Xoptimus
# Generated by qmake (3.1) (Qt 6.7.1)
# Project:  Xoptimus.pro
# Template: app
# Command: /usr/bin/qmake -o Makefile Xoptimus.pro
#############################################################################

MAKEFILE      = Makefile

EQ            = =

####### Compiler, tools and options

CC            = gcc
CXX           = g++
DEFINES       = -DQT_NO_DEBUG -DQT_WIDGETS_LIB -DQT_GUI_LIB -DQT_CORE_LIB
CFLAGS        = -pipe -O2 -O3 -march=native -fopenmp -unroll-loops -omit-frame-pointer -Winline -unsafe-math-optimizations -mtune=native -pthread -Wall -Wextra -D_REENTRANT $(DEFINES)
CXXFLAGS      = -pipe -O2 -O3 -fopenmp -unroll-loops -fomit-frame-pointer -Winline -unsafe-math-optimizations -mtune=native -pthread -std=gnu++1z -Wall -Wextra -D_REENTRANT $(DEFINES)
INCPATH       = -I. -I/usr/include/qt6 -I/usr/include/qt6/QtWidgets -I/usr/include/qt6/QtGui -I/usr/include/qt6/QtCore -I. -I/usr/lib64/qt6/mkspecs/linux-g++
QMAKE         = /usr/bin/qmake
DEL_FILE      = rm -f
CHK_DIR_EXISTS= test -d
MKDIR         = mkdir -p
COPY          = cp -f
COPY_FILE     = cp -f
COPY_DIR      = cp -f -R
INSTALL_FILE  = install -m 644 -p
INSTALL_PROGRAM = install -m 755 -p
INSTALL_DIR   = cp -f -R
QINSTALL      = /usr/bin/qmake -install qinstall
QINSTALL_PROGRAM = /usr/bin/qmake -install qinstall -exe
DEL_FILE      = rm -f
SYMLINK       = ln -f -s
DEL_DIR       = rmdir
MOVE          = mv -f
TAR           = tar -cf
COMPRESS      = gzip -9f
DISTNAME      = Xoptimus1.0.0
DISTDIR = /home/sheridan/Desktop/ERGASIES/GlobalOptimus/.tmp/Xoptimus1.0.0
LINK          = g++
LFLAGS        = -O3 -Wl,-O1 -Wl,-rpath-link,/usr/lib64
LIBS          = $(SUBLIBS) -lm -fopenmp /usr/lib64/libQt6Widgets.so /usr/lib64/libQt6Gui.so /usr/lib64/libQt6Core.so -lpthread -lGLX -lOpenGL   
AR            = ar cqs
RANLIB        = 
SED           = sed
STRIP         = strip

####### Output directory

OBJECTS_DIR   = ./

####### Files

SOURCES       = GE/cprogram.cc \
		GE/doublestack.cc \
		GE/fparser.cc \
		GE/fpoptimizer.cc \
		GE/integeranneal.cpp \
		GE/nncprogram.cpp \
		GE/population.cc \
		GE/program.cc \
		GE/rule.cc \
		GE/ruleprogram.cpp \
		GE/symbol.cc \
		LINESEARCH/armijosearch.cpp \
		LINESEARCH/fibonaccisearch.cpp \
		LINESEARCH/goldensearch.cpp \
		LINESEARCH/linesearch.cpp \
		METHODS/adam.cpp \
		METHODS/armadillo1.cpp \
		METHODS/bfgs.cpp \
		METHODS/differentialevolution.cpp \
		METHODS/genetic.cpp \
		METHODS/gradientdescent.cpp \
		METHODS/gwooptimizer.cpp \
		METHODS/ipso.cpp \
		METHODS/lbfgs.cpp \
		METHODS/multistart.cpp \
		METHODS/neldermead.cpp \
		METHODS/neuralminimizer.cpp \
		METHODS/ofalgorithm.cpp \
		METHODS/parallelde.cpp \
		METHODS/parallelpso.cpp \
		METHODS/simanmethod.cpp \
		METHODS/usermethod.cpp \
		MLMODELS/converter.cc \
		MLMODELS/dataset.cpp \
		MLMODELS/gdfmodel.cpp \
		MLMODELS/mlpproblem.cpp \
		MLMODELS/model.cpp \
		MLMODELS/neuralparser.cc \
		MLMODELS/nncmodel.cpp \
		MLMODELS/rbfproblem.cpp \
		MLMODELS/rulemodel.cpp \
		OPTIMUS/collection.cpp \
		OPTIMUS/doublebox.cpp \
		OPTIMUS/optimizer.cpp \
		OPTIMUS/parameter.cpp \
		OPTIMUS/parameterlist.cpp \
		OPTIMUS/problem.cpp \
		XOPTIMUS/methodloader.cpp \
		XOPTIMUS/parameterdialog.cpp \
		XOPTIMUS/problemloader.cpp \
		OPTIMUS/similarity.cpp \
		OPTIMUS/statistics.cpp \
		PROBLEMS/bf1.cpp \
		PROBLEMS/bf2.cpp \
		PROBLEMS/branin.cpp \
		PROBLEMS/camel.cpp \
		PROBLEMS/cm.cpp \
		PROBLEMS/diffpower.cpp \
		PROBLEMS/diracproblem.cpp \
		PROBLEMS/easom.cpp \
		PROBLEMS/elp.cpp \
		PROBLEMS/exp.cpp \
		PROBLEMS/fuch1.cpp \
		PROBLEMS/fuchss.cpp \
		PROBLEMS/gkls.cpp \
		PROBLEMS/gkls2100.cpp \
		PROBLEMS/gkls250.cpp \
		PROBLEMS/gkls350.cpp \
		PROBLEMS/goldstein.cpp \
		PROBLEMS/griewank10.cpp \
		PROBLEMS/griewank2.cpp \
		PROBLEMS/gross.cpp \
		PROBLEMS/hansen.cpp \
		PROBLEMS/hartman3.cpp \
		PROBLEMS/hartman6.cpp \
		PROBLEMS/hess.cpp \
		PROBLEMS/levy.cpp \
		PROBLEMS/potential.cpp \
		PROBLEMS/rastriginproblem.cpp \
		PROBLEMS/rosenbrock.cpp \
		PROBLEMS/salkin.cpp \
		PROBLEMS/shekel10.cpp \
		PROBLEMS/shekel5.cpp \
		PROBLEMS/shekel7.cpp \
		PROBLEMS/sinu.cpp \
		PROBLEMS/test2nproblem.cpp \
		PROBLEMS/test30n.cpp \
		PROBLEMS/userproblem.cpp \
		SAMPLER/kmeanssampler.cpp \
		SAMPLER/maxwellsampler.cpp \
		SAMPLER/neuralsampler.cpp \
		SAMPLER/problemsampler.cpp \
		SAMPLER/rbfsampler.cpp \
		SAMPLER/triangularsampler.cpp \
		SAMPLER/uniformsampler.cpp \
		XOPTIMUS/selectmethoddialog.cpp \
		XOPTIMUS/selectproblemdialog.cpp \
		XOPTIMUS/xoptimusmain.cpp \
		XOPTIMUS/mainwindow.cpp qrc_resources.cpp \
		moc_parameterdialog.cpp \
		moc_mainwindow.cpp \
		moc_selectmethoddialog.cpp \
		moc_selectproblemdialog.cpp
OBJECTS       = cprogram.o \
		doublestack.o \
		fparser.o \
		fpoptimizer.o \
		integeranneal.o \
		nncprogram.o \
		population.o \
		program.o \
		rule.o \
		ruleprogram.o \
		symbol.o \
		armijosearch.o \
		fibonaccisearch.o \
		goldensearch.o \
		linesearch.o \
		adam.o \
		armadillo1.o \
		bfgs.o \
		differentialevolution.o \
		genetic.o \
		gradientdescent.o \
		gwooptimizer.o \
		ipso.o \
		lbfgs.o \
		multistart.o \
		neldermead.o \
		neuralminimizer.o \
		ofalgorithm.o \
		parallelde.o \
		parallelpso.o \
		simanmethod.o \
		usermethod.o \
		converter.o \
		dataset.o \
		gdfmodel.o \
		mlpproblem.o \
		model.o \
		neuralparser.o \
		nncmodel.o \
		rbfproblem.o \
		rulemodel.o \
		collection.o \
		doublebox.o \
		optimizer.o \
		parameter.o \
		parameterlist.o \
		problem.o \
		methodloader.o \
		parameterdialog.o \
		problemloader.o \
		similarity.o \
		statistics.o \
		bf1.o \
		bf2.o \
		branin.o \
		camel.o \
		cm.o \
		diffpower.o \
		diracproblem.o \
		easom.o \
		elp.o \
		exp.o \
		fuch1.o \
		fuchss.o \
		gkls.o \
		gkls2100.o \
		gkls250.o \
		gkls350.o \
		goldstein.o \
		griewank10.o \
		griewank2.o \
		gross.o \
		hansen.o \
		hartman3.o \
		hartman6.o \
		hess.o \
		levy.o \
		potential.o \
		rastriginproblem.o \
		rosenbrock.o \
		salkin.o \
		shekel10.o \
		shekel5.o \
		shekel7.o \
		sinu.o \
		test2nproblem.o \
		test30n.o \
		userproblem.o \
		kmeanssampler.o \
		maxwellsampler.o \
		neuralsampler.o \
		problemsampler.o \
		rbfsampler.o \
		triangularsampler.o \
		uniformsampler.o \
		selectmethoddialog.o \
		selectproblemdialog.o \
		xoptimusmain.o \
		mainwindow.o \
		qrc_resources.o \
		moc_parameterdialog.o \
		moc_mainwindow.o \
		moc_selectmethoddialog.o \
		moc_selectproblemdialog.o
DIST          = /usr/lib64/qt6/mkspecs/features/spec_pre.prf \
		/usr/lib64/qt6/mkspecs/common/unix.conf \
		/usr/lib64/qt6/mkspecs/common/linux.conf \
		/usr/lib64/qt6/mkspecs/common/sanitize.conf \
		/usr/lib64/qt6/mkspecs/common/gcc-base.conf \
		/usr/lib64/qt6/mkspecs/common/gcc-base-unix.conf \
		/usr/lib64/qt6/mkspecs/common/g++-base.conf \
		/usr/lib64/qt6/mkspecs/common/g++-unix.conf \
		/usr/lib64/qt6/mkspecs/qconfig.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_concurrent.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_concurrent_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_core.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_core_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_dbus.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_dbus_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_devicediscovery_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfs_kms_gbm_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfs_kms_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfsdeviceintegration_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_example_icons_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_fb_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_gui.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_gui_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_input_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_kms_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_network.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_network_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_opengl.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_opengl_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_openglwidgets.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_openglwidgets_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_printsupport.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_printsupport_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_sql.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_sql_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_testlib.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_testlib_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_widgets.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_widgets_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_xcb_qpa_lib_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_xml.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_xml_private.pri \
		/usr/lib64/qt6/mkspecs/features/qt_functions.prf \
		/usr/lib64/qt6/mkspecs/features/qt_config.prf \
		/usr/lib64/qt6/mkspecs/linux-g++/qmake.conf \
		/usr/lib64/qt6/mkspecs/features/spec_post.prf \
		.qmake.stash \
		/usr/lib64/qt6/mkspecs/features/exclusive_builds.prf \
		/usr/lib64/qt6/mkspecs/features/toolchain.prf \
		/usr/lib64/qt6/mkspecs/features/default_pre.prf \
		/usr/lib64/qt6/mkspecs/features/resolve_config.prf \
		/usr/lib64/qt6/mkspecs/features/default_post.prf \
		/usr/lib64/qt6/mkspecs/features/warn_on.prf \
		/usr/lib64/qt6/mkspecs/features/permissions.prf \
		/usr/lib64/qt6/mkspecs/features/qt.prf \
		/usr/lib64/qt6/mkspecs/features/resources_functions.prf \
		/usr/lib64/qt6/mkspecs/features/resources.prf \
		/usr/lib64/qt6/mkspecs/features/moc.prf \
		/usr/lib64/qt6/mkspecs/features/unix/opengl.prf \
		/usr/lib64/qt6/mkspecs/features/uic.prf \
		/usr/lib64/qt6/mkspecs/features/unix/thread.prf \
		/usr/lib64/qt6/mkspecs/features/qmake_use.prf \
		/usr/lib64/qt6/mkspecs/features/file_copies.prf \
		/usr/lib64/qt6/mkspecs/features/testcase_targets.prf \
		/usr/lib64/qt6/mkspecs/features/exceptions.prf \
		/usr/lib64/qt6/mkspecs/features/yacc.prf \
		/usr/lib64/qt6/mkspecs/features/lex.prf \
		Xoptimus.pro GE/cprogram.h \
		GE/doublestack.h \
		GE/fparser.hh \
		GE/fpconfig.hh \
		GE/fptypes.hh \
		GE/integeranneal.h \
		GE/nncprogram.h \
		GE/population.h \
		GE/program.h \
		GE/rule.h \
		GE/ruleprogram.h \
		GE/symbol.h \
		LINESEARCH/armijosearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/linesearch.h \
		METHODS/adam.h \
		METHODS/armadillo1.h \
		METHODS/bfgs.h \
		METHODS/differentialevolution.h \
		METHODS/genetic.h \
		METHODS/gradientdescent.h \
		METHODS/gwooptimizer.h \
		METHODS/ipso.h \
		METHODS/lbfgs.h \
		METHODS/multistart.h \
		METHODS/neldermead.h \
		METHODS/neuralminimizer.h \
		METHODS/ofalgorithm.h \
		METHODS/parallelde.h \
		METHODS/parallelpso.h \
		METHODS/simanmethod.h \
		METHODS/usermethod.h \
		MLMODELS/converter.h \
		MLMODELS/dataset.h \
		MLMODELS/functionalrbf.h \
		MLMODELS/gdfmodel.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/model.h \
		MLMODELS/neuralparser.h \
		MLMODELS/nncmodel.h \
		MLMODELS/rbfproblem.h \
		MLMODELS/rulemodel.h \
		OPTIMUS/collection.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/problem.h \
		XOPTIMUS/methodloader.h \
		XOPTIMUS/parameterdialog.h \
		XOPTIMUS/problemloader.h \
		OPTIMUS/similarity.h \
		OPTIMUS/statistics.h \
		PROBLEMS/bf1.h \
		PROBLEMS/bf2.h \
		PROBLEMS/branin.h \
		PROBLEMS/camel.h \
		PROBLEMS/cm.h \
		PROBLEMS/diffpower.h \
		PROBLEMS/diracproblem.h \
		PROBLEMS/easom.h \
		PROBLEMS/elp.h \
		PROBLEMS/exp.h \
		PROBLEMS/fuch1.h \
		PROBLEMS/fuchss.h \
		PROBLEMS/gkls.h \
		PROBLEMS/gkls2100.h \
		PROBLEMS/gkls250.h \
		PROBLEMS/gkls350.h \
		PROBLEMS/goldstein.h \
		PROBLEMS/griewank10.h \
		PROBLEMS/griewank2.h \
		PROBLEMS/gross.h \
		PROBLEMS/hansen.h \
		PROBLEMS/hartman3.h \
		PROBLEMS/hartman6.h \
		PROBLEMS/hess.h \
		PROBLEMS/levy.h \
		PROBLEMS/potential.h \
		PROBLEMS/rastriginproblem.h \
		PROBLEMS/rosenbrock.h \
		PROBLEMS/salkin.h \
		PROBLEMS/shekel10.h \
		PROBLEMS/shekel5.h \
		PROBLEMS/shekel7.h \
		PROBLEMS/sinu.h \
		PROBLEMS/test2nproblem.h \
		PROBLEMS/test30n.h \
		PROBLEMS/userproblem.h \
		SAMPLER/kmeanssampler.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/neuralsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		XOPTIMUS/mainwindow.h \
		XOPTIMUS/selectmethoddialog.h \
		XOPTIMUS/selectproblemdialog.h GE/cprogram.cc \
		GE/doublestack.cc \
		GE/fparser.cc \
		GE/fpoptimizer.cc \
		GE/integeranneal.cpp \
		GE/nncprogram.cpp \
		GE/population.cc \
		GE/program.cc \
		GE/rule.cc \
		GE/ruleprogram.cpp \
		GE/symbol.cc \
		LINESEARCH/armijosearch.cpp \
		LINESEARCH/fibonaccisearch.cpp \
		LINESEARCH/goldensearch.cpp \
		LINESEARCH/linesearch.cpp \
		METHODS/adam.cpp \
		METHODS/armadillo1.cpp \
		METHODS/bfgs.cpp \
		METHODS/differentialevolution.cpp \
		METHODS/genetic.cpp \
		METHODS/gradientdescent.cpp \
		METHODS/gwooptimizer.cpp \
		METHODS/ipso.cpp \
		METHODS/lbfgs.cpp \
		METHODS/multistart.cpp \
		METHODS/neldermead.cpp \
		METHODS/neuralminimizer.cpp \
		METHODS/ofalgorithm.cpp \
		METHODS/parallelde.cpp \
		METHODS/parallelpso.cpp \
		METHODS/simanmethod.cpp \
		METHODS/usermethod.cpp \
		MLMODELS/converter.cc \
		MLMODELS/dataset.cpp \
		MLMODELS/gdfmodel.cpp \
		MLMODELS/mlpproblem.cpp \
		MLMODELS/model.cpp \
		MLMODELS/neuralparser.cc \
		MLMODELS/nncmodel.cpp \
		MLMODELS/rbfproblem.cpp \
		MLMODELS/rulemodel.cpp \
		OPTIMUS/collection.cpp \
		OPTIMUS/doublebox.cpp \
		OPTIMUS/optimizer.cpp \
		OPTIMUS/parameter.cpp \
		OPTIMUS/parameterlist.cpp \
		OPTIMUS/problem.cpp \
		XOPTIMUS/methodloader.cpp \
		XOPTIMUS/parameterdialog.cpp \
		XOPTIMUS/problemloader.cpp \
		OPTIMUS/similarity.cpp \
		OPTIMUS/statistics.cpp \
		PROBLEMS/bf1.cpp \
		PROBLEMS/bf2.cpp \
		PROBLEMS/branin.cpp \
		PROBLEMS/camel.cpp \
		PROBLEMS/cm.cpp \
		PROBLEMS/diffpower.cpp \
		PROBLEMS/diracproblem.cpp \
		PROBLEMS/easom.cpp \
		PROBLEMS/elp.cpp \
		PROBLEMS/exp.cpp \
		PROBLEMS/fuch1.cpp \
		PROBLEMS/fuchss.cpp \
		PROBLEMS/gkls.cpp \
		PROBLEMS/gkls2100.cpp \
		PROBLEMS/gkls250.cpp \
		PROBLEMS/gkls350.cpp \
		PROBLEMS/goldstein.cpp \
		PROBLEMS/griewank10.cpp \
		PROBLEMS/griewank2.cpp \
		PROBLEMS/gross.cpp \
		PROBLEMS/hansen.cpp \
		PROBLEMS/hartman3.cpp \
		PROBLEMS/hartman6.cpp \
		PROBLEMS/hess.cpp \
		PROBLEMS/levy.cpp \
		PROBLEMS/potential.cpp \
		PROBLEMS/rastriginproblem.cpp \
		PROBLEMS/rosenbrock.cpp \
		PROBLEMS/salkin.cpp \
		PROBLEMS/shekel10.cpp \
		PROBLEMS/shekel5.cpp \
		PROBLEMS/shekel7.cpp \
		PROBLEMS/sinu.cpp \
		PROBLEMS/test2nproblem.cpp \
		PROBLEMS/test30n.cpp \
		PROBLEMS/userproblem.cpp \
		SAMPLER/kmeanssampler.cpp \
		SAMPLER/maxwellsampler.cpp \
		SAMPLER/neuralsampler.cpp \
		SAMPLER/problemsampler.cpp \
		SAMPLER/rbfsampler.cpp \
		SAMPLER/triangularsampler.cpp \
		SAMPLER/uniformsampler.cpp \
		XOPTIMUS/selectmethoddialog.cpp \
		XOPTIMUS/selectproblemdialog.cpp \
		XOPTIMUS/xoptimusmain.cpp \
		XOPTIMUS/mainwindow.cpp
QMAKE_TARGET  = Xoptimus
DESTDIR       = 
TARGET        = Xoptimus


first: all
####### Build rules

Xoptimus:  $(OBJECTS)  
	$(LINK) $(LFLAGS) -o $(TARGET)  $(OBJECTS) $(OBJCOMP) $(LIBS)

Makefile: Xoptimus.pro /usr/lib64/qt6/mkspecs/linux-g++/qmake.conf /usr/lib64/qt6/mkspecs/features/spec_pre.prf \
		/usr/lib64/qt6/mkspecs/common/unix.conf \
		/usr/lib64/qt6/mkspecs/common/linux.conf \
		/usr/lib64/qt6/mkspecs/common/sanitize.conf \
		/usr/lib64/qt6/mkspecs/common/gcc-base.conf \
		/usr/lib64/qt6/mkspecs/common/gcc-base-unix.conf \
		/usr/lib64/qt6/mkspecs/common/g++-base.conf \
		/usr/lib64/qt6/mkspecs/common/g++-unix.conf \
		/usr/lib64/qt6/mkspecs/qconfig.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_concurrent.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_concurrent_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_core.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_core_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_dbus.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_dbus_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_devicediscovery_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfs_kms_gbm_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfs_kms_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfsdeviceintegration_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_example_icons_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_fb_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_gui.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_gui_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_input_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_kms_support_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_network.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_network_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_opengl.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_opengl_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_openglwidgets.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_openglwidgets_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_printsupport.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_printsupport_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_sql.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_sql_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_testlib.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_testlib_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_widgets.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_widgets_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_xcb_qpa_lib_private.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_xml.pri \
		/usr/lib64/qt6/mkspecs/modules/qt_lib_xml_private.pri \
		/usr/lib64/qt6/mkspecs/features/qt_functions.prf \
		/usr/lib64/qt6/mkspecs/features/qt_config.prf \
		/usr/lib64/qt6/mkspecs/linux-g++/qmake.conf \
		/usr/lib64/qt6/mkspecs/features/spec_post.prf \
		.qmake.stash \
		/usr/lib64/qt6/mkspecs/features/exclusive_builds.prf \
		/usr/lib64/qt6/mkspecs/features/toolchain.prf \
		/usr/lib64/qt6/mkspecs/features/default_pre.prf \
		/usr/lib64/qt6/mkspecs/features/resolve_config.prf \
		/usr/lib64/qt6/mkspecs/features/default_post.prf \
		/usr/lib64/qt6/mkspecs/features/warn_on.prf \
		/usr/lib64/qt6/mkspecs/features/permissions.prf \
		/usr/lib64/qt6/mkspecs/features/qt.prf \
		/usr/lib64/qt6/mkspecs/features/resources_functions.prf \
		/usr/lib64/qt6/mkspecs/features/resources.prf \
		/usr/lib64/qt6/mkspecs/features/moc.prf \
		/usr/lib64/qt6/mkspecs/features/unix/opengl.prf \
		/usr/lib64/qt6/mkspecs/features/uic.prf \
		/usr/lib64/qt6/mkspecs/features/unix/thread.prf \
		/usr/lib64/qt6/mkspecs/features/qmake_use.prf \
		/usr/lib64/qt6/mkspecs/features/file_copies.prf \
		/usr/lib64/qt6/mkspecs/features/testcase_targets.prf \
		/usr/lib64/qt6/mkspecs/features/exceptions.prf \
		/usr/lib64/qt6/mkspecs/features/yacc.prf \
		/usr/lib64/qt6/mkspecs/features/lex.prf \
		Xoptimus.pro \
		resources.qrc \
		/usr/lib64/libQt6Widgets.prl \
		/usr/lib64/libQt6Gui.prl \
		/usr/lib64/libQt6Core.prl
	$(QMAKE) -o Makefile Xoptimus.pro
/usr/lib64/qt6/mkspecs/features/spec_pre.prf:
/usr/lib64/qt6/mkspecs/common/unix.conf:
/usr/lib64/qt6/mkspecs/common/linux.conf:
/usr/lib64/qt6/mkspecs/common/sanitize.conf:
/usr/lib64/qt6/mkspecs/common/gcc-base.conf:
/usr/lib64/qt6/mkspecs/common/gcc-base-unix.conf:
/usr/lib64/qt6/mkspecs/common/g++-base.conf:
/usr/lib64/qt6/mkspecs/common/g++-unix.conf:
/usr/lib64/qt6/mkspecs/qconfig.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_concurrent.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_concurrent_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_core.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_core_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_dbus.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_dbus_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_devicediscovery_support_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfs_kms_gbm_support_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfs_kms_support_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_eglfsdeviceintegration_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_example_icons_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_fb_support_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_gui.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_gui_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_input_support_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_kms_support_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_network.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_network_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_opengl.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_opengl_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_openglwidgets.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_openglwidgets_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_printsupport.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_printsupport_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_sql.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_sql_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_testlib.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_testlib_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_widgets.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_widgets_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_xcb_qpa_lib_private.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_xml.pri:
/usr/lib64/qt6/mkspecs/modules/qt_lib_xml_private.pri:
/usr/lib64/qt6/mkspecs/features/qt_functions.prf:
/usr/lib64/qt6/mkspecs/features/qt_config.prf:
/usr/lib64/qt6/mkspecs/linux-g++/qmake.conf:
/usr/lib64/qt6/mkspecs/features/spec_post.prf:
.qmake.stash:
/usr/lib64/qt6/mkspecs/features/exclusive_builds.prf:
/usr/lib64/qt6/mkspecs/features/toolchain.prf:
/usr/lib64/qt6/mkspecs/features/default_pre.prf:
/usr/lib64/qt6/mkspecs/features/resolve_config.prf:
/usr/lib64/qt6/mkspecs/features/default_post.prf:
/usr/lib64/qt6/mkspecs/features/warn_on.prf:
/usr/lib64/qt6/mkspecs/features/permissions.prf:
/usr/lib64/qt6/mkspecs/features/qt.prf:
/usr/lib64/qt6/mkspecs/features/resources_functions.prf:
/usr/lib64/qt6/mkspecs/features/resources.prf:
/usr/lib64/qt6/mkspecs/features/moc.prf:
/usr/lib64/qt6/mkspecs/features/unix/opengl.prf:
/usr/lib64/qt6/mkspecs/features/uic.prf:
/usr/lib64/qt6/mkspecs/features/unix/thread.prf:
/usr/lib64/qt6/mkspecs/features/qmake_use.prf:
/usr/lib64/qt6/mkspecs/features/file_copies.prf:
/usr/lib64/qt6/mkspecs/features/testcase_targets.prf:
/usr/lib64/qt6/mkspecs/features/exceptions.prf:
/usr/lib64/qt6/mkspecs/features/yacc.prf:
/usr/lib64/qt6/mkspecs/features/lex.prf:
Xoptimus.pro:
resources.qrc:
/usr/lib64/libQt6Widgets.prl:
/usr/lib64/libQt6Gui.prl:
/usr/lib64/libQt6Core.prl:
qmake: FORCE
	@$(QMAKE) -o Makefile Xoptimus.pro

qmake_all: FORCE


all: Makefile Xoptimus

dist: distdir FORCE
	(cd `dirname $(DISTDIR)` && $(TAR) $(DISTNAME).tar $(DISTNAME) && $(COMPRESS) $(DISTNAME).tar) && $(MOVE) `dirname $(DISTDIR)`/$(DISTNAME).tar.gz . && $(DEL_FILE) -r $(DISTDIR)

distdir: FORCE
	@test -d $(DISTDIR) || mkdir -p $(DISTDIR)
	$(COPY_FILE) --parents $(DIST) $(DISTDIR)/
	$(COPY_FILE) --parents resources.qrc $(DISTDIR)/
	$(COPY_FILE) --parents /usr/lib64/qt6/mkspecs/features/data/dummy.cpp $(DISTDIR)/
	$(COPY_FILE) --parents GE/cprogram.h GE/doublestack.h GE/fparser.hh GE/fpconfig.hh GE/fptypes.hh GE/integeranneal.h GE/nncprogram.h GE/population.h GE/program.h GE/rule.h GE/ruleprogram.h GE/symbol.h LINESEARCH/armijosearch.h LINESEARCH/fibonaccisearch.h LINESEARCH/goldensearch.h LINESEARCH/linesearch.h METHODS/adam.h METHODS/armadillo1.h METHODS/bfgs.h METHODS/differentialevolution.h METHODS/genetic.h METHODS/gradientdescent.h METHODS/gwooptimizer.h METHODS/ipso.h METHODS/lbfgs.h METHODS/multistart.h METHODS/neldermead.h METHODS/neuralminimizer.h METHODS/ofalgorithm.h METHODS/parallelde.h METHODS/parallelpso.h METHODS/simanmethod.h METHODS/usermethod.h MLMODELS/converter.h MLMODELS/dataset.h MLMODELS/functionalrbf.h MLMODELS/gdfmodel.h MLMODELS/mlpproblem.h MLMODELS/model.h MLMODELS/neuralparser.h MLMODELS/nncmodel.h MLMODELS/rbfproblem.h MLMODELS/rulemodel.h OPTIMUS/collection.h OPTIMUS/doublebox.h OPTIMUS/optimizer.h OPTIMUS/parameter.h OPTIMUS/parameterlist.h OPTIMUS/problem.h XOPTIMUS/methodloader.h XOPTIMUS/parameterdialog.h XOPTIMUS/problemloader.h OPTIMUS/similarity.h OPTIMUS/statistics.h PROBLEMS/bf1.h PROBLEMS/bf2.h PROBLEMS/branin.h PROBLEMS/camel.h PROBLEMS/cm.h PROBLEMS/diffpower.h PROBLEMS/diracproblem.h PROBLEMS/easom.h PROBLEMS/elp.h PROBLEMS/exp.h PROBLEMS/fuch1.h PROBLEMS/fuchss.h PROBLEMS/gkls.h PROBLEMS/gkls2100.h PROBLEMS/gkls250.h PROBLEMS/gkls350.h PROBLEMS/goldstein.h PROBLEMS/griewank10.h PROBLEMS/griewank2.h PROBLEMS/gross.h PROBLEMS/hansen.h PROBLEMS/hartman3.h PROBLEMS/hartman6.h PROBLEMS/hess.h PROBLEMS/levy.h PROBLEMS/potential.h PROBLEMS/rastriginproblem.h PROBLEMS/rosenbrock.h PROBLEMS/salkin.h PROBLEMS/shekel10.h PROBLEMS/shekel5.h PROBLEMS/shekel7.h PROBLEMS/sinu.h PROBLEMS/test2nproblem.h PROBLEMS/test30n.h PROBLEMS/userproblem.h SAMPLER/kmeanssampler.h SAMPLER/maxwellsampler.h SAMPLER/neuralsampler.h SAMPLER/problemsampler.h SAMPLER/rbfsampler.h SAMPLER/triangularsampler.h SAMPLER/uniformsampler.h XOPTIMUS/mainwindow.h XOPTIMUS/selectmethoddialog.h XOPTIMUS/selectproblemdialog.h $(DISTDIR)/
	$(COPY_FILE) --parents GE/cprogram.cc GE/doublestack.cc GE/fparser.cc GE/fpoptimizer.cc GE/integeranneal.cpp GE/nncprogram.cpp GE/population.cc GE/program.cc GE/rule.cc GE/ruleprogram.cpp GE/symbol.cc LINESEARCH/armijosearch.cpp LINESEARCH/fibonaccisearch.cpp LINESEARCH/goldensearch.cpp LINESEARCH/linesearch.cpp METHODS/adam.cpp METHODS/armadillo1.cpp METHODS/bfgs.cpp METHODS/differentialevolution.cpp METHODS/genetic.cpp METHODS/gradientdescent.cpp METHODS/gwooptimizer.cpp METHODS/ipso.cpp METHODS/lbfgs.cpp METHODS/multistart.cpp METHODS/neldermead.cpp METHODS/neuralminimizer.cpp METHODS/ofalgorithm.cpp METHODS/parallelde.cpp METHODS/parallelpso.cpp METHODS/simanmethod.cpp METHODS/usermethod.cpp MLMODELS/converter.cc MLMODELS/dataset.cpp MLMODELS/gdfmodel.cpp MLMODELS/mlpproblem.cpp MLMODELS/model.cpp MLMODELS/neuralparser.cc MLMODELS/nncmodel.cpp MLMODELS/rbfproblem.cpp MLMODELS/rulemodel.cpp OPTIMUS/collection.cpp OPTIMUS/doublebox.cpp OPTIMUS/optimizer.cpp OPTIMUS/parameter.cpp OPTIMUS/parameterlist.cpp OPTIMUS/problem.cpp XOPTIMUS/methodloader.cpp XOPTIMUS/parameterdialog.cpp XOPTIMUS/problemloader.cpp OPTIMUS/similarity.cpp OPTIMUS/statistics.cpp PROBLEMS/bf1.cpp PROBLEMS/bf2.cpp PROBLEMS/branin.cpp PROBLEMS/camel.cpp PROBLEMS/cm.cpp PROBLEMS/diffpower.cpp PROBLEMS/diracproblem.cpp PROBLEMS/easom.cpp PROBLEMS/elp.cpp PROBLEMS/exp.cpp PROBLEMS/fuch1.cpp PROBLEMS/fuchss.cpp PROBLEMS/gkls.cpp PROBLEMS/gkls2100.cpp PROBLEMS/gkls250.cpp PROBLEMS/gkls350.cpp PROBLEMS/goldstein.cpp PROBLEMS/griewank10.cpp PROBLEMS/griewank2.cpp PROBLEMS/gross.cpp PROBLEMS/hansen.cpp PROBLEMS/hartman3.cpp PROBLEMS/hartman6.cpp PROBLEMS/hess.cpp PROBLEMS/levy.cpp PROBLEMS/potential.cpp PROBLEMS/rastriginproblem.cpp PROBLEMS/rosenbrock.cpp PROBLEMS/salkin.cpp PROBLEMS/shekel10.cpp PROBLEMS/shekel5.cpp PROBLEMS/shekel7.cpp PROBLEMS/sinu.cpp PROBLEMS/test2nproblem.cpp PROBLEMS/test30n.cpp PROBLEMS/userproblem.cpp SAMPLER/kmeanssampler.cpp SAMPLER/maxwellsampler.cpp SAMPLER/neuralsampler.cpp SAMPLER/problemsampler.cpp SAMPLER/rbfsampler.cpp SAMPLER/triangularsampler.cpp SAMPLER/uniformsampler.cpp XOPTIMUS/selectmethoddialog.cpp XOPTIMUS/selectproblemdialog.cpp XOPTIMUS/xoptimusmain.cpp XOPTIMUS/mainwindow.cpp $(DISTDIR)/


clean: compiler_clean 
	-$(DEL_FILE) $(OBJECTS)
	-$(DEL_FILE) *~ core *.core


distclean: clean 
	-$(DEL_FILE) $(TARGET) 
	-$(DEL_FILE) .qmake.stash
	-$(DEL_FILE) Makefile


####### Sub-libraries

mocclean: compiler_moc_header_clean compiler_moc_objc_header_clean compiler_moc_source_clean

mocables: compiler_moc_header_make_all compiler_moc_objc_header_make_all compiler_moc_source_make_all

check: first

benchmark: first

compiler_rcc_make_all: qrc_resources.cpp
compiler_rcc_clean:
	-$(DEL_FILE) qrc_resources.cpp
qrc_resources.cpp: resources.qrc \
		/usr/lib64/qt6/libexec/rcc \
		XOPTIMUS/android_icon.png \
		XOPTIMUS/windows_icon.png \
		XOPTIMUS/update.png \
		XOPTIMUS/state_wait.png \
		XOPTIMUS/state_pause.png \
		XOPTIMUS/apple_icon.png \
		XOPTIMUS/state_run.png \
		XOPTIMUS/state_finish.png \
		XOPTIMUS/add.png \
		XOPTIMUS/minus.png \
		XOPTIMUS/trash.png \
		XOPTIMUS/raspberry.png
	/usr/lib64/qt6/libexec/rcc -name resources resources.qrc -o qrc_resources.cpp

compiler_moc_predefs_make_all: moc_predefs.h
compiler_moc_predefs_clean:
	-$(DEL_FILE) moc_predefs.h
moc_predefs.h: /usr/lib64/qt6/mkspecs/features/data/dummy.cpp
	g++ -pipe -O2 -O3 -fopenmp -unroll-loops -fomit-frame-pointer -Winline -unsafe-math-optimizations -mtune=native -pthread -std=gnu++1z -Wall -Wextra -dM -E -o moc_predefs.h /usr/lib64/qt6/mkspecs/features/data/dummy.cpp

compiler_moc_header_make_all: moc_parameterdialog.cpp moc_mainwindow.cpp moc_selectmethoddialog.cpp moc_selectproblemdialog.cpp
compiler_moc_header_clean:
	-$(DEL_FILE) moc_parameterdialog.cpp moc_mainwindow.cpp moc_selectmethoddialog.cpp moc_selectproblemdialog.cpp
moc_parameterdialog.cpp: XOPTIMUS/parameterdialog.h \
		moc_predefs.h \
		/usr/lib64/qt6/libexec/moc
	/usr/lib64/qt6/libexec/moc $(DEFINES) --include /home/sheridan/Desktop/ERGASIES/GlobalOptimus/moc_predefs.h -I/usr/lib64/qt6/mkspecs/linux-g++ -I/home/sheridan/Desktop/ERGASIES/GlobalOptimus -I/usr/include/qt6 -I/usr/include/qt6/QtWidgets -I/usr/include/qt6/QtGui -I/usr/include/qt6/QtCore -I/usr/include/c++/12 -I/usr/include/x86_64-linux-gnu/c++/12 -I/usr/include/c++/12/backward -I/usr/lib/gcc/x86_64-linux-gnu/12/include -I/usr/local/include -I/usr/include/x86_64-linux-gnu -I/usr/include XOPTIMUS/parameterdialog.h -o moc_parameterdialog.cpp

moc_mainwindow.cpp: XOPTIMUS/mainwindow.h \
		XOPTIMUS/parameterdialog.h \
		XOPTIMUS/problemloader.h \
		OPTIMUS/problem.h \
		PROBLEMS/rastriginproblem.h \
		PROBLEMS/test2nproblem.h \
		PROBLEMS/bf1.h \
		PROBLEMS/bf2.h \
		PROBLEMS/branin.h \
		PROBLEMS/camel.h \
		PROBLEMS/cm.h \
		PROBLEMS/diffpower.h \
		PROBLEMS/easom.h \
		PROBLEMS/elp.h \
		PROBLEMS/exp.h \
		PROBLEMS/gkls250.h \
		PROBLEMS/gkls2100.h \
		PROBLEMS/gkls350.h \
		PROBLEMS/goldstein.h \
		PROBLEMS/griewank2.h \
		PROBLEMS/griewank10.h \
		PROBLEMS/hansen.h \
		PROBLEMS/hartman3.h \
		PROBLEMS/hartman6.h \
		PROBLEMS/potential.h \
		PROBLEMS/rosenbrock.h \
		PROBLEMS/gross.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/dataset.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		MLMODELS/rbfproblem.h \
		PROBLEMS/shekel5.h \
		PROBLEMS/shekel7.h \
		PROBLEMS/shekel10.h \
		PROBLEMS/sinu.h \
		PROBLEMS/test30n.h \
		PROBLEMS/diracproblem.h \
		PROBLEMS/userproblem.h \
		PROBLEMS/fuchss.h \
		PROBLEMS/levy.h \
		PROBLEMS/salkin.h \
		PROBLEMS/hess.h \
		PROBLEMS/fuch1.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		XOPTIMUS/selectproblemdialog.h \
		XOPTIMUS/methodloader.h \
		METHODS/gradientdescent.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/armijosearch.h \
		METHODS/multistart.h \
		OPTIMUS/collection.h \
		METHODS/genetic.h \
		METHODS/bfgs.h \
		METHODS/lbfgs.h \
		METHODS/differentialevolution.h \
		METHODS/ipso.h \
		METHODS/neuralminimizer.h \
		METHODS/parallelde.h \
		METHODS/parallelpso.h \
		METHODS/neldermead.h \
		METHODS/adam.h \
		METHODS/armadillo1.h \
		METHODS/simanmethod.h \
		METHODS/usermethod.h \
		METHODS/ofalgorithm.h \
		METHODS/gwooptimizer.h \
		XOPTIMUS/selectmethoddialog.h \
		OPTIMUS/statistics.h \
		moc_predefs.h \
		/usr/lib64/qt6/libexec/moc
	/usr/lib64/qt6/libexec/moc $(DEFINES) --include /home/sheridan/Desktop/ERGASIES/GlobalOptimus/moc_predefs.h -I/usr/lib64/qt6/mkspecs/linux-g++ -I/home/sheridan/Desktop/ERGASIES/GlobalOptimus -I/usr/include/qt6 -I/usr/include/qt6/QtWidgets -I/usr/include/qt6/QtGui -I/usr/include/qt6/QtCore -I/usr/include/c++/12 -I/usr/include/x86_64-linux-gnu/c++/12 -I/usr/include/c++/12/backward -I/usr/lib/gcc/x86_64-linux-gnu/12/include -I/usr/local/include -I/usr/include/x86_64-linux-gnu -I/usr/include XOPTIMUS/mainwindow.h -o moc_mainwindow.cpp

moc_selectmethoddialog.cpp: XOPTIMUS/selectmethoddialog.h \
		moc_predefs.h \
		/usr/lib64/qt6/libexec/moc
	/usr/lib64/qt6/libexec/moc $(DEFINES) --include /home/sheridan/Desktop/ERGASIES/GlobalOptimus/moc_predefs.h -I/usr/lib64/qt6/mkspecs/linux-g++ -I/home/sheridan/Desktop/ERGASIES/GlobalOptimus -I/usr/include/qt6 -I/usr/include/qt6/QtWidgets -I/usr/include/qt6/QtGui -I/usr/include/qt6/QtCore -I/usr/include/c++/12 -I/usr/include/x86_64-linux-gnu/c++/12 -I/usr/include/c++/12/backward -I/usr/lib/gcc/x86_64-linux-gnu/12/include -I/usr/local/include -I/usr/include/x86_64-linux-gnu -I/usr/include XOPTIMUS/selectmethoddialog.h -o moc_selectmethoddialog.cpp

moc_selectproblemdialog.cpp: XOPTIMUS/selectproblemdialog.h \
		moc_predefs.h \
		/usr/lib64/qt6/libexec/moc
	/usr/lib64/qt6/libexec/moc $(DEFINES) --include /home/sheridan/Desktop/ERGASIES/GlobalOptimus/moc_predefs.h -I/usr/lib64/qt6/mkspecs/linux-g++ -I/home/sheridan/Desktop/ERGASIES/GlobalOptimus -I/usr/include/qt6 -I/usr/include/qt6/QtWidgets -I/usr/include/qt6/QtGui -I/usr/include/qt6/QtCore -I/usr/include/c++/12 -I/usr/include/x86_64-linux-gnu/c++/12 -I/usr/include/c++/12/backward -I/usr/lib/gcc/x86_64-linux-gnu/12/include -I/usr/local/include -I/usr/include/x86_64-linux-gnu -I/usr/include XOPTIMUS/selectproblemdialog.h -o moc_selectproblemdialog.cpp

compiler_moc_objc_header_make_all:
compiler_moc_objc_header_clean:
compiler_moc_source_make_all:
compiler_moc_source_clean:
compiler_uic_make_all:
compiler_uic_clean:
compiler_yacc_decl_make_all:
compiler_yacc_decl_clean:
compiler_yacc_impl_make_all:
compiler_yacc_impl_clean:
compiler_lex_make_all:
compiler_lex_clean:
compiler_clean: compiler_rcc_clean compiler_moc_predefs_clean compiler_moc_header_clean 

####### Compile

cprogram.o: GE/cprogram.cc GE/cprogram.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h \
		GE/fparser.hh \
		GE/program.h \
		MLMODELS/model.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o cprogram.o GE/cprogram.cc

doublestack.o: GE/doublestack.cc GE/doublestack.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o doublestack.o GE/doublestack.cc

fparser.o: GE/fparser.cc GE/fparser.hh \
		GE/doublestack.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o fparser.o GE/fparser.cc

fpoptimizer.o: GE/fpoptimizer.cc GE/fpconfig.hh \
		GE/fparser.hh \
		GE/fptypes.hh
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o fpoptimizer.o GE/fpoptimizer.cc

integeranneal.o: GE/integeranneal.cpp GE/integeranneal.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		GE/program.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o integeranneal.o GE/integeranneal.cpp

nncprogram.o: GE/nncprogram.cpp GE/nncprogram.h \
		GE/cprogram.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h \
		GE/fparser.hh \
		GE/program.h \
		MLMODELS/model.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o nncprogram.o GE/nncprogram.cpp

population.o: GE/population.cc GE/population.h \
		GE/program.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h \
		OPTIMUS/problem.h \
		METHODS/bfgs.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		METHODS/simanmethod.h \
		OPTIMUS/collection.h \
		GE/integeranneal.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o population.o GE/population.cc

program.o: GE/program.cc GE/program.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o program.o GE/program.cc

rule.o: GE/rule.cc GE/rule.h \
		GE/symbol.h \
		GE/doublestack.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o rule.o GE/rule.cc

ruleprogram.o: GE/ruleprogram.cpp GE/ruleprogram.h \
		GE/cprogram.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h \
		GE/fparser.hh \
		GE/program.h \
		MLMODELS/model.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o ruleprogram.o GE/ruleprogram.cpp

symbol.o: GE/symbol.cc GE/symbol.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o symbol.o GE/symbol.cc

armijosearch.o: LINESEARCH/armijosearch.cpp LINESEARCH/armijosearch.h \
		LINESEARCH/linesearch.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o armijosearch.o LINESEARCH/armijosearch.cpp

fibonaccisearch.o: LINESEARCH/fibonaccisearch.cpp LINESEARCH/fibonaccisearch.h \
		LINESEARCH/linesearch.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o fibonaccisearch.o LINESEARCH/fibonaccisearch.cpp

goldensearch.o: LINESEARCH/goldensearch.cpp LINESEARCH/goldensearch.h \
		LINESEARCH/linesearch.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o goldensearch.o LINESEARCH/goldensearch.cpp

linesearch.o: LINESEARCH/linesearch.cpp LINESEARCH/linesearch.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o linesearch.o LINESEARCH/linesearch.cpp

adam.o: METHODS/adam.cpp METHODS/adam.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/armijosearch.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o adam.o METHODS/adam.cpp

armadillo1.o: METHODS/armadillo1.cpp METHODS/armadillo1.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o armadillo1.o METHODS/armadillo1.cpp

bfgs.o: METHODS/bfgs.cpp METHODS/bfgs.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o bfgs.o METHODS/bfgs.cpp

differentialevolution.o: METHODS/differentialevolution.cpp METHODS/differentialevolution.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o differentialevolution.o METHODS/differentialevolution.cpp

genetic.o: METHODS/genetic.cpp METHODS/genetic.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o genetic.o METHODS/genetic.cpp

gradientdescent.o: METHODS/gradientdescent.cpp METHODS/gradientdescent.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/armijosearch.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gradientdescent.o METHODS/gradientdescent.cpp

gwooptimizer.o: METHODS/gwooptimizer.cpp METHODS/gwooptimizer.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gwooptimizer.o METHODS/gwooptimizer.cpp

ipso.o: METHODS/ipso.cpp METHODS/ipso.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o ipso.o METHODS/ipso.cpp

lbfgs.o: METHODS/lbfgs.cpp METHODS/lbfgs.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o lbfgs.o METHODS/lbfgs.cpp

multistart.o: METHODS/multistart.cpp METHODS/multistart.h \
		OPTIMUS/problem.h \
		OPTIMUS/collection.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o multistart.o METHODS/multistart.cpp

neldermead.o: METHODS/neldermead.cpp METHODS/neldermead.h \
		OPTIMUS/problem.h \
		OPTIMUS/collection.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o neldermead.o METHODS/neldermead.cpp

neuralminimizer.o: METHODS/neuralminimizer.cpp METHODS/neuralminimizer.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o neuralminimizer.o METHODS/neuralminimizer.cpp

ofalgorithm.o: METHODS/ofalgorithm.cpp METHODS/ofalgorithm.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		METHODS/gradientdescent.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/armijosearch.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o ofalgorithm.o METHODS/ofalgorithm.cpp

parallelde.o: METHODS/parallelde.cpp METHODS/parallelde.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o parallelde.o METHODS/parallelde.cpp

parallelpso.o: METHODS/parallelpso.cpp METHODS/parallelpso.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o parallelpso.o METHODS/parallelpso.cpp

simanmethod.o: METHODS/simanmethod.cpp METHODS/simanmethod.h \
		OPTIMUS/collection.h \
		OPTIMUS/problem.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o simanmethod.o METHODS/simanmethod.cpp

usermethod.o: METHODS/usermethod.cpp METHODS/usermethod.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o usermethod.o METHODS/usermethod.cpp

converter.o: MLMODELS/converter.cc MLMODELS/converter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o converter.o MLMODELS/converter.cc

dataset.o: MLMODELS/dataset.cpp MLMODELS/dataset.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o dataset.o MLMODELS/dataset.cpp

gdfmodel.o: MLMODELS/gdfmodel.cpp MLMODELS/gdfmodel.h \
		MLMODELS/model.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		GE/cprogram.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h \
		GE/fparser.hh \
		GE/program.h \
		GE/population.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gdfmodel.o MLMODELS/gdfmodel.cpp

mlpproblem.o: MLMODELS/mlpproblem.cpp MLMODELS/mlpproblem.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o mlpproblem.o MLMODELS/mlpproblem.cpp

model.o: MLMODELS/model.cpp MLMODELS/model.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o model.o MLMODELS/model.cpp

neuralparser.o: MLMODELS/neuralparser.cc MLMODELS/neuralparser.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o neuralparser.o MLMODELS/neuralparser.cc

nncmodel.o: MLMODELS/nncmodel.cpp MLMODELS/nncmodel.h \
		MLMODELS/model.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		GE/nncprogram.h \
		GE/cprogram.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h \
		GE/fparser.hh \
		GE/program.h \
		GE/population.h \
		MLMODELS/neuralparser.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/converter.h \
		METHODS/bfgs.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		METHODS/lbfgs.h \
		METHODS/adam.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/armijosearch.h \
		METHODS/genetic.h \
		OPTIMUS/collection.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o nncmodel.o MLMODELS/nncmodel.cpp

rbfproblem.o: MLMODELS/rbfproblem.cpp MLMODELS/rbfproblem.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o rbfproblem.o MLMODELS/rbfproblem.cpp

rulemodel.o: MLMODELS/rulemodel.cpp MLMODELS/rulemodel.h \
		MLMODELS/model.h \
		MLMODELS/dataset.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		GE/nncprogram.h \
		GE/cprogram.h \
		GE/symbol.h \
		GE/rule.h \
		GE/doublestack.h \
		GE/fparser.hh \
		GE/program.h \
		GE/population.h \
		MLMODELS/neuralparser.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/converter.h \
		METHODS/bfgs.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		METHODS/lbfgs.h \
		METHODS/adam.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/armijosearch.h \
		METHODS/genetic.h \
		OPTIMUS/collection.h \
		GE/ruleprogram.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o rulemodel.o MLMODELS/rulemodel.cpp

collection.o: OPTIMUS/collection.cpp OPTIMUS/collection.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o collection.o OPTIMUS/collection.cpp

doublebox.o: OPTIMUS/doublebox.cpp OPTIMUS/doublebox.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o doublebox.o OPTIMUS/doublebox.cpp

optimizer.o: OPTIMUS/optimizer.cpp OPTIMUS/optimizer.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		METHODS/gradientdescent.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/armijosearch.h \
		METHODS/bfgs.h \
		METHODS/lbfgs.h \
		METHODS/neldermead.h \
		OPTIMUS/collection.h \
		METHODS/adam.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o optimizer.o OPTIMUS/optimizer.cpp

parameter.o: OPTIMUS/parameter.cpp OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o parameter.o OPTIMUS/parameter.cpp

parameterlist.o: OPTIMUS/parameterlist.cpp OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o parameterlist.o OPTIMUS/parameterlist.cpp

problem.o: OPTIMUS/problem.cpp OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o problem.o OPTIMUS/problem.cpp

methodloader.o: XOPTIMUS/methodloader.cpp XOPTIMUS/methodloader.h \
		METHODS/gradientdescent.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/problem.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		MLMODELS/dataset.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/armijosearch.h \
		METHODS/multistart.h \
		OPTIMUS/collection.h \
		METHODS/genetic.h \
		METHODS/bfgs.h \
		METHODS/lbfgs.h \
		METHODS/differentialevolution.h \
		METHODS/ipso.h \
		METHODS/neuralminimizer.h \
		METHODS/parallelde.h \
		METHODS/parallelpso.h \
		METHODS/neldermead.h \
		METHODS/adam.h \
		METHODS/armadillo1.h \
		METHODS/simanmethod.h \
		METHODS/usermethod.h \
		METHODS/ofalgorithm.h \
		METHODS/gwooptimizer.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o methodloader.o XOPTIMUS/methodloader.cpp

parameterdialog.o: XOPTIMUS/parameterdialog.cpp XOPTIMUS/parameterdialog.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o parameterdialog.o XOPTIMUS/parameterdialog.cpp

problemloader.o: XOPTIMUS/problemloader.cpp XOPTIMUS/problemloader.h \
		OPTIMUS/problem.h \
		PROBLEMS/rastriginproblem.h \
		PROBLEMS/test2nproblem.h \
		PROBLEMS/bf1.h \
		PROBLEMS/bf2.h \
		PROBLEMS/branin.h \
		PROBLEMS/camel.h \
		PROBLEMS/cm.h \
		PROBLEMS/diffpower.h \
		PROBLEMS/easom.h \
		PROBLEMS/elp.h \
		PROBLEMS/exp.h \
		PROBLEMS/gkls250.h \
		PROBLEMS/gkls2100.h \
		PROBLEMS/gkls350.h \
		PROBLEMS/goldstein.h \
		PROBLEMS/griewank2.h \
		PROBLEMS/griewank10.h \
		PROBLEMS/hansen.h \
		PROBLEMS/hartman3.h \
		PROBLEMS/hartman6.h \
		PROBLEMS/potential.h \
		PROBLEMS/rosenbrock.h \
		PROBLEMS/gross.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/dataset.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		MLMODELS/rbfproblem.h \
		PROBLEMS/shekel5.h \
		PROBLEMS/shekel7.h \
		PROBLEMS/shekel10.h \
		PROBLEMS/sinu.h \
		PROBLEMS/test30n.h \
		PROBLEMS/diracproblem.h \
		PROBLEMS/userproblem.h \
		PROBLEMS/fuchss.h \
		PROBLEMS/levy.h \
		PROBLEMS/salkin.h \
		PROBLEMS/hess.h \
		PROBLEMS/fuch1.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o problemloader.o XOPTIMUS/problemloader.cpp

similarity.o: OPTIMUS/similarity.cpp OPTIMUS/similarity.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o similarity.o OPTIMUS/similarity.cpp

statistics.o: OPTIMUS/statistics.cpp OPTIMUS/statistics.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o statistics.o OPTIMUS/statistics.cpp

bf1.o: PROBLEMS/bf1.cpp PROBLEMS/bf1.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o bf1.o PROBLEMS/bf1.cpp

bf2.o: PROBLEMS/bf2.cpp PROBLEMS/bf2.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o bf2.o PROBLEMS/bf2.cpp

branin.o: PROBLEMS/branin.cpp PROBLEMS/branin.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o branin.o PROBLEMS/branin.cpp

camel.o: PROBLEMS/camel.cpp PROBLEMS/camel.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o camel.o PROBLEMS/camel.cpp

cm.o: PROBLEMS/cm.cpp PROBLEMS/cm.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o cm.o PROBLEMS/cm.cpp

diffpower.o: PROBLEMS/diffpower.cpp PROBLEMS/diffpower.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o diffpower.o PROBLEMS/diffpower.cpp

diracproblem.o: PROBLEMS/diracproblem.cpp PROBLEMS/diracproblem.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o diracproblem.o PROBLEMS/diracproblem.cpp

easom.o: PROBLEMS/easom.cpp PROBLEMS/easom.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o easom.o PROBLEMS/easom.cpp

elp.o: PROBLEMS/elp.cpp PROBLEMS/elp.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o elp.o PROBLEMS/elp.cpp

exp.o: PROBLEMS/exp.cpp PROBLEMS/exp.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o exp.o PROBLEMS/exp.cpp

fuch1.o: PROBLEMS/fuch1.cpp PROBLEMS/fuch1.h \
		OPTIMUS/problem.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/dataset.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o fuch1.o PROBLEMS/fuch1.cpp

fuchss.o: PROBLEMS/fuchss.cpp PROBLEMS/fuchss.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o fuchss.o PROBLEMS/fuchss.cpp

gkls.o: PROBLEMS/gkls.cpp PROBLEMS/gkls.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gkls.o PROBLEMS/gkls.cpp

gkls2100.o: PROBLEMS/gkls2100.cpp PROBLEMS/gkls2100.h \
		OPTIMUS/problem.h \
		PROBLEMS/gkls.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gkls2100.o PROBLEMS/gkls2100.cpp

gkls250.o: PROBLEMS/gkls250.cpp PROBLEMS/gkls250.h \
		OPTIMUS/problem.h \
		PROBLEMS/gkls.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gkls250.o PROBLEMS/gkls250.cpp

gkls350.o: PROBLEMS/gkls350.cpp PROBLEMS/gkls350.h \
		OPTIMUS/problem.h \
		PROBLEMS/gkls.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gkls350.o PROBLEMS/gkls350.cpp

goldstein.o: PROBLEMS/goldstein.cpp PROBLEMS/goldstein.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o goldstein.o PROBLEMS/goldstein.cpp

griewank10.o: PROBLEMS/griewank10.cpp PROBLEMS/griewank10.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o griewank10.o PROBLEMS/griewank10.cpp

griewank2.o: PROBLEMS/griewank2.cpp PROBLEMS/griewank2.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o griewank2.o PROBLEMS/griewank2.cpp

gross.o: PROBLEMS/gross.cpp PROBLEMS/gross.h \
		OPTIMUS/problem.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/dataset.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		MLMODELS/rbfproblem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o gross.o PROBLEMS/gross.cpp

hansen.o: PROBLEMS/hansen.cpp PROBLEMS/hansen.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o hansen.o PROBLEMS/hansen.cpp

hartman3.o: PROBLEMS/hartman3.cpp PROBLEMS/hartman3.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o hartman3.o PROBLEMS/hartman3.cpp

hartman6.o: PROBLEMS/hartman6.cpp PROBLEMS/hartman6.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o hartman6.o PROBLEMS/hartman6.cpp

hess.o: PROBLEMS/hess.cpp PROBLEMS/hess.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o hess.o PROBLEMS/hess.cpp

levy.o: PROBLEMS/levy.cpp PROBLEMS/levy.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o levy.o PROBLEMS/levy.cpp

potential.o: PROBLEMS/potential.cpp PROBLEMS/potential.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o potential.o PROBLEMS/potential.cpp

rastriginproblem.o: PROBLEMS/rastriginproblem.cpp PROBLEMS/rastriginproblem.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o rastriginproblem.o PROBLEMS/rastriginproblem.cpp

rosenbrock.o: PROBLEMS/rosenbrock.cpp PROBLEMS/rosenbrock.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o rosenbrock.o PROBLEMS/rosenbrock.cpp

salkin.o: PROBLEMS/salkin.cpp PROBLEMS/salkin.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o salkin.o PROBLEMS/salkin.cpp

shekel10.o: PROBLEMS/shekel10.cpp PROBLEMS/shekel10.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o shekel10.o PROBLEMS/shekel10.cpp

shekel5.o: PROBLEMS/shekel5.cpp PROBLEMS/shekel5.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o shekel5.o PROBLEMS/shekel5.cpp

shekel7.o: PROBLEMS/shekel7.cpp PROBLEMS/shekel7.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o shekel7.o PROBLEMS/shekel7.cpp

sinu.o: PROBLEMS/sinu.cpp PROBLEMS/sinu.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o sinu.o PROBLEMS/sinu.cpp

test2nproblem.o: PROBLEMS/test2nproblem.cpp PROBLEMS/test2nproblem.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o test2nproblem.o PROBLEMS/test2nproblem.cpp

test30n.o: PROBLEMS/test30n.cpp PROBLEMS/test30n.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o test30n.o PROBLEMS/test30n.cpp

userproblem.o: PROBLEMS/userproblem.cpp PROBLEMS/userproblem.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o userproblem.o PROBLEMS/userproblem.cpp

kmeanssampler.o: SAMPLER/kmeanssampler.cpp SAMPLER/kmeanssampler.h \
		SAMPLER/problemsampler.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o kmeanssampler.o SAMPLER/kmeanssampler.cpp

maxwellsampler.o: SAMPLER/maxwellsampler.cpp SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o maxwellsampler.o SAMPLER/maxwellsampler.cpp

neuralsampler.o: SAMPLER/neuralsampler.cpp SAMPLER/neuralsampler.h \
		SAMPLER/problemsampler.h \
		OPTIMUS/problem.h \
		MLMODELS/dataset.h \
		METHODS/bfgs.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/parameter.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		METHODS/lbfgs.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o neuralsampler.o SAMPLER/neuralsampler.cpp

problemsampler.o: SAMPLER/problemsampler.cpp SAMPLER/problemsampler.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o problemsampler.o SAMPLER/problemsampler.cpp

rbfsampler.o: SAMPLER/rbfsampler.cpp SAMPLER/rbfsampler.h \
		OPTIMUS/problem.h \
		SAMPLER/problemsampler.h \
		MLMODELS/dataset.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o rbfsampler.o SAMPLER/rbfsampler.cpp

triangularsampler.o: SAMPLER/triangularsampler.cpp SAMPLER/triangularsampler.h \
		SAMPLER/problemsampler.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o triangularsampler.o SAMPLER/triangularsampler.cpp

uniformsampler.o: SAMPLER/uniformsampler.cpp SAMPLER/uniformsampler.h \
		SAMPLER/problemsampler.h \
		OPTIMUS/problem.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o uniformsampler.o SAMPLER/uniformsampler.cpp

selectmethoddialog.o: XOPTIMUS/selectmethoddialog.cpp XOPTIMUS/selectmethoddialog.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o selectmethoddialog.o XOPTIMUS/selectmethoddialog.cpp

selectproblemdialog.o: XOPTIMUS/selectproblemdialog.cpp XOPTIMUS/selectproblemdialog.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o selectproblemdialog.o XOPTIMUS/selectproblemdialog.cpp

xoptimusmain.o: XOPTIMUS/xoptimusmain.cpp XOPTIMUS/mainwindow.h \
		XOPTIMUS/parameterdialog.h \
		XOPTIMUS/problemloader.h \
		OPTIMUS/problem.h \
		PROBLEMS/rastriginproblem.h \
		PROBLEMS/test2nproblem.h \
		PROBLEMS/bf1.h \
		PROBLEMS/bf2.h \
		PROBLEMS/branin.h \
		PROBLEMS/camel.h \
		PROBLEMS/cm.h \
		PROBLEMS/diffpower.h \
		PROBLEMS/easom.h \
		PROBLEMS/elp.h \
		PROBLEMS/exp.h \
		PROBLEMS/gkls250.h \
		PROBLEMS/gkls2100.h \
		PROBLEMS/gkls350.h \
		PROBLEMS/goldstein.h \
		PROBLEMS/griewank2.h \
		PROBLEMS/griewank10.h \
		PROBLEMS/hansen.h \
		PROBLEMS/hartman3.h \
		PROBLEMS/hartman6.h \
		PROBLEMS/potential.h \
		PROBLEMS/rosenbrock.h \
		PROBLEMS/gross.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/dataset.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		MLMODELS/rbfproblem.h \
		PROBLEMS/shekel5.h \
		PROBLEMS/shekel7.h \
		PROBLEMS/shekel10.h \
		PROBLEMS/sinu.h \
		PROBLEMS/test30n.h \
		PROBLEMS/diracproblem.h \
		PROBLEMS/userproblem.h \
		PROBLEMS/fuchss.h \
		PROBLEMS/levy.h \
		PROBLEMS/salkin.h \
		PROBLEMS/hess.h \
		PROBLEMS/fuch1.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		XOPTIMUS/selectproblemdialog.h \
		XOPTIMUS/methodloader.h \
		METHODS/gradientdescent.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/armijosearch.h \
		METHODS/multistart.h \
		OPTIMUS/collection.h \
		METHODS/genetic.h \
		METHODS/bfgs.h \
		METHODS/lbfgs.h \
		METHODS/differentialevolution.h \
		METHODS/ipso.h \
		METHODS/neuralminimizer.h \
		METHODS/parallelde.h \
		METHODS/parallelpso.h \
		METHODS/neldermead.h \
		METHODS/adam.h \
		METHODS/armadillo1.h \
		METHODS/simanmethod.h \
		METHODS/usermethod.h \
		METHODS/ofalgorithm.h \
		METHODS/gwooptimizer.h \
		XOPTIMUS/selectmethoddialog.h \
		OPTIMUS/statistics.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o xoptimusmain.o XOPTIMUS/xoptimusmain.cpp

mainwindow.o: XOPTIMUS/mainwindow.cpp XOPTIMUS/mainwindow.h \
		XOPTIMUS/parameterdialog.h \
		XOPTIMUS/problemloader.h \
		OPTIMUS/problem.h \
		PROBLEMS/rastriginproblem.h \
		PROBLEMS/test2nproblem.h \
		PROBLEMS/bf1.h \
		PROBLEMS/bf2.h \
		PROBLEMS/branin.h \
		PROBLEMS/camel.h \
		PROBLEMS/cm.h \
		PROBLEMS/diffpower.h \
		PROBLEMS/easom.h \
		PROBLEMS/elp.h \
		PROBLEMS/exp.h \
		PROBLEMS/gkls250.h \
		PROBLEMS/gkls2100.h \
		PROBLEMS/gkls350.h \
		PROBLEMS/goldstein.h \
		PROBLEMS/griewank2.h \
		PROBLEMS/griewank10.h \
		PROBLEMS/hansen.h \
		PROBLEMS/hartman3.h \
		PROBLEMS/hartman6.h \
		PROBLEMS/potential.h \
		PROBLEMS/rosenbrock.h \
		PROBLEMS/gross.h \
		MLMODELS/mlpproblem.h \
		MLMODELS/dataset.h \
		MLMODELS/model.h \
		OPTIMUS/parameterlist.h \
		OPTIMUS/parameter.h \
		MLMODELS/rbfproblem.h \
		PROBLEMS/shekel5.h \
		PROBLEMS/shekel7.h \
		PROBLEMS/shekel10.h \
		PROBLEMS/sinu.h \
		PROBLEMS/test30n.h \
		PROBLEMS/diracproblem.h \
		PROBLEMS/userproblem.h \
		PROBLEMS/fuchss.h \
		PROBLEMS/levy.h \
		PROBLEMS/salkin.h \
		PROBLEMS/hess.h \
		PROBLEMS/fuch1.h \
		OPTIMUS/optimizer.h \
		OPTIMUS/doublebox.h \
		OPTIMUS/similarity.h \
		SAMPLER/maxwellsampler.h \
		SAMPLER/problemsampler.h \
		SAMPLER/neuralsampler.h \
		SAMPLER/triangularsampler.h \
		SAMPLER/uniformsampler.h \
		SAMPLER/rbfsampler.h \
		SAMPLER/kmeanssampler.h \
		XOPTIMUS/selectproblemdialog.h \
		XOPTIMUS/methodloader.h \
		METHODS/gradientdescent.h \
		LINESEARCH/linesearch.h \
		LINESEARCH/fibonaccisearch.h \
		LINESEARCH/goldensearch.h \
		LINESEARCH/armijosearch.h \
		METHODS/multistart.h \
		OPTIMUS/collection.h \
		METHODS/genetic.h \
		METHODS/bfgs.h \
		METHODS/lbfgs.h \
		METHODS/differentialevolution.h \
		METHODS/ipso.h \
		METHODS/neuralminimizer.h \
		METHODS/parallelde.h \
		METHODS/parallelpso.h \
		METHODS/neldermead.h \
		METHODS/adam.h \
		METHODS/armadillo1.h \
		METHODS/simanmethod.h \
		METHODS/usermethod.h \
		METHODS/ofalgorithm.h \
		METHODS/gwooptimizer.h \
		XOPTIMUS/selectmethoddialog.h \
		OPTIMUS/statistics.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o mainwindow.o XOPTIMUS/mainwindow.cpp

qrc_resources.o: qrc_resources.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o qrc_resources.o qrc_resources.cpp

moc_parameterdialog.o: moc_parameterdialog.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_parameterdialog.o moc_parameterdialog.cpp

moc_mainwindow.o: moc_mainwindow.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_mainwindow.o moc_mainwindow.cpp

moc_selectmethoddialog.o: moc_selectmethoddialog.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_selectmethoddialog.o moc_selectmethoddialog.cpp

moc_selectproblemdialog.o: moc_selectproblemdialog.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_selectproblemdialog.o moc_selectproblemdialog.cpp

####### Install

install_target: first FORCE
	@test -d $(INSTALL_ROOT)/opt/Xoptimus/bin || mkdir -p $(INSTALL_ROOT)/opt/Xoptimus/bin
	$(QINSTALL_PROGRAM) $(QMAKE_TARGET) $(INSTALL_ROOT)/opt/Xoptimus/bin/$(QMAKE_TARGET)
	-$(STRIP) $(INSTALL_ROOT)/opt/Xoptimus/bin/$(QMAKE_TARGET)

uninstall_target: FORCE
	-$(DEL_FILE) $(INSTALL_ROOT)/opt/Xoptimus/bin/$(QMAKE_TARGET)
	-$(DEL_DIR) $(INSTALL_ROOT)/opt/Xoptimus/bin/ 


install: install_target  FORCE

uninstall: uninstall_target  FORCE

FORCE:

.SUFFIXES:

