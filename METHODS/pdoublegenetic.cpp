...
pDoubleGenetic::pDoubleGenetic()
{
    ...
    addParam(Parameter("pgen_chromosomes", 100,10,10000, "Number of chromosomes"));
    addParam(Parameter("pgen_generations", 200,10,10000, "Number of generations"));
    addParam(Parameter("pgen_selectionrate", 0.10,0.0,1.0,"Selection rate"));
    addParam(Parameter("pgen_mutationrate", 0.05,0.0,1.0, "Mutation rate"));
    addParam(Parameter("pgen_localsearchrate", 0.00,0.0,1.0, "Local search rate"));
    addParam(Parameter("pgen_subClusterEnable", "1", "the number of subclusters that play a role in the termination rule: [1, islands]"));
    addParam(Parameter("pgen_subCluster", "5", "number of subclusters for pDoubleGenetic"));
    addParam(Parameter("pgen_centers", "20", "centers"));
    addParam(Parameter("pgen_pNumber", "5", "the number of samples for propagation"));
    QStringList pmethod;
...
{
    QString stop_method=terminationMethod;
    double difference = fabs(bestF2xInClusterOLD.at(subClusterName) - bestF2xInCluster.at(subClusterName));
    similarityMaxCount = getParam("opt_similaritycriterion").getValue().toInt();
...
void pDoubleGenetic::step()
{
    ...
    double rate = getParam("pgen_localsearchrate").getValue().toDouble();
    dmin = 1e+100;
    QString debug = getParam("opt_debug").getValue();
...
{
    rate = getParam("pgen_localsearchrate").getValue().toDouble();
    debug = getParam("opt_debug").getValue();
    sample_method = getParam("pgen_sampler").getValue();
    propagateRate = getParam("pgen_propagateRate").getValue().toInt();
    parallelPropagateMethod = getParam("pgen_parallelPropagateMethod").getValue();
    pNumber = getParam("pgen_pNumber").getValue().toInt();
    mutation_rate = getParam("pgen_mutationrate").getValue().toDouble();
    selection_rate = getParam("pgen_selectionrate").getValue().toDouble();
    centers = getParam("pgen_centers").getValue().toInt();
    double_chromosomes = getParam("pgen_chromosomes").getValue().toInt();
    double_generations = getParam("pgen_generations").getValue().toInt();
    subCluster = getParam("pgen_subCluster").getValue().toInt();
    lmargin = myProblem->getLeftMargin();
    rmargin = myProblem->getRightMargin();
    similarityCurrentCount.resize(subCluster);
    subClusterEnable = getParam("pgen_subClusterEnable").getValue().toInt();
    ...
    if (sample_method == "teams")
    {
        ...
        {
            Point point(i, myProblem->getSample());
    ...
    else if(sample_method=="uniform")
    {
        ...
        for (int i = 0; i < population; i++)
        {
            chromosome[i] = myProblem->getSample();
            fitnessArray[i] = myProblem->funmin(chromosome[i]);
        }
