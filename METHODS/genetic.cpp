...
Genetic::Genetic()
{
    ...
    addParam(Parameter("gen_mutation","random","Mutation method. Available values random, double"));
    ...
    for(int i=startPos;i<chromosomeCount;i++)
    {
        for(int j=0;j<myProblem->getDimension();j++)
        {
            ...
            if(r<mutationRate)
            {
                ...
                {
                    double delta = 2.0*(rand()*1.0/RAND_MAX)-1.0;
                    delta = delta * x[j]/100.0;
