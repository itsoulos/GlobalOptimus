...
class LocalRbfProblem : public Problem
{
private:
    RbfSampler *thisSampler;
public:
    LocalRbfProblem(RbfSampler *p);
    virtual double      funmin(Data &x);
    virtual Data gradient(Data &x);
    ~LocalRbfProblem();
};

LocalRbfProblem::LocalRbfProblem(RbfSampler *p)
    :Problem(p->getParameterSize())
{
    thisSampler = p;
...
double      LocalRbfProblem::funmin(Data &x)
{
...
Data        LocalRbfProblem::gradient(Data &x)
{
    thisSampler->setParameters(x);
    Data g;
    g.resize(x.size());
...
LocalRbfProblem::~LocalRbfProblem()
{
    //nothing for now
}
