...
optimum of a problem. According to the literature survey, there are
a variety of real-world problems that can be applied in mathematics
\citep{go_math1,go_math2,go_math3}, physics \citep{go_physics1,go_physics2,go_physics3},
...
they approach solving the problem. The techniques belong to the category
of deterministic nethods are mainly the so - called interval methods
\citep{interval1,interval2}. Stochastic methods utilize randomness
to explore the solution space, while in interval methods, the set
$S$ is divided into smaller regions that may contain the global minimum
based on certain criteria. Recently, a comparison between deterministic
and stochastic methods was proposed by Sergeyev et al \citep{Sergeyev}.
...
Also, due to the wide spread of parallel computing units\citep{key-7}\citep{key-8}\textbf{,
...
Genetic algorithms have been used widely in areas such as networking
\citep{ga_problem1}, robotics \citep{ga_problem2,ga_problem3}, energy
topics \citep{ga_problem4,ga_problem5} etc.\textbf{ }They can be
combined with machine learning to solve complex problems, such as
neural network training \citep{ga_nn1,ga_nn2}.
...
of Armijo\citep{armijo}. The solutions produced in the previous step
are used to formulate new trial solutions using a process derived
...
sample $c_{i}$ is identified from the initial set $x_{i}$ \ref{eq:nearestSamply}.
Then, a second sample is generated using the Armijo linear search,
following the termination criterion of the method \ref{eq:findSample-1},
considering both the initial and the nearest sample. A third sample
is subsequently computed through the “crossover” mechanism of Genetic
Algorithms \ref{eq:makeChild}, combining the initial sample with
the best one obtained from the entire search space. These samples
are used to create a new trial sample based on the formula \ref{eq:trialSample},
leveraging both local information and the position of the best sample.The
periodic application of a local optimization method further improves
the algorithm’s performance by ensuring solution refinement. The termination
criterion is controlled through an adaptive process, determining whether
the search should continue or stop based on the achieved progress.
In the conducted tests, the algorithm demonstrated high efficiency
by significantly reducing the number of objective function evaluations
and the overall computational cost. The algorithm is as follows:
\begin{enumerate}
...
\item \textbf{Set} direction vectors: $p_{1}=-\nabla fs(x_{i})$ end $p_{2}=-\nabla fs(c_{i})$
\item \textbf{Set} initial step size for Armijo $a=a_{0}$
\item \textbf{Find }new points using line search Armijo $\mbox{minLS}(x_{i},c_{i})$:
...
\par\end{centering}
\caption{The flowchart of the proposed optimization procedure.\label{fig:flow}}
...
as suggested by Shen Y. et al \citep{charilogis}to the objective
problems.
...
\begin{table}[H]
\begin{centering}
{\footnotesize{}\caption{Comparison of average function calls of proposed method against others\label{tab:mainTable}}
}{\footnotesize\par}
\par\end{centering}
...
\begin{centering}
\caption{Function calls with different differential weight F\label{tab:differentF}}
\begin{tabular}{|c|c|c|c|}
\hline 
FUNCTION & Proposed (F:random) & Proposed (F:0.5) & Proposed (F:0.9)\tabularnewline
\hline 
\hline 
ACKLAY & 5818 & 5206 & 5379\tabularnewline
\hline 
...
\begin{figure}[H]
\centering{}\subfloat[Comparison of function calls with outliers appearing\label{fig:CompWith}]{
\begin{centering}
\includegraphics[scale=0.4]{new10}
\par\end{centering}
}\subfloat[Comparison of function calls without outliers appearing\label{fig:CompWithout}]{\begin{centering}
\includegraphics[scale=0.4]{new11}
\par\end{centering}
}\caption{Comparison using the number of function calls. The test was performed
for different optimization methods.\label{fig:proposedVSOthers}}
\end{figure}
 

\begin{figure}[H]
\centering{}\includegraphics[scale=0.5]{new12}\caption{Statistical comparison of proposed method against others. The test
was performed for different optimization methods\label{fig:StatProposedVSOthers}}
\end{figure}
...
\begin{center}
\begin{figure}[H]
\centering{}\subfloat[Comparison of the function calls for the ELP function of the proposed
method with other methods\label{fig:ELPcalls}]{\centering{}\includegraphics[scale=0.4]{new13}}\subfloat[Time comparison for the ELP function of the proposed method with other
methods\label{fig:ELPTimes}]{\begin{centering}
\includegraphics[scale=0.4]{new14}
\par\end{centering}
}\caption{Different variations of the ELP function of the proposed method with
other methods\label{fig:ELP}}
\end{figure}
\par\end{center}
...
\centering{}\includegraphics[scale=0.4]{new4}}\caption{Performance comparison of proposed methods against differential evolution:
Effect of K-Means algorithm on convergence.\label{fig:proposedsVSDE}}
\end{figure}
\par\end{center}
...
These candidate solutions are then combined to cr\label{fig:ELPcalls-1}eate
new solutions utilizing approaches inspired by the Differential Evolution
method. To validate the effectiveness of this new optimization approach,
a comprehensive series of experiments were conducted on various problems
...
\bibitem{go_chem3}Jeraal, M. I., Sung, S., \& Lapkin, A. A. (2021).
A Machine Learning‐Enabled Autonomous Flow Chemistry Platform for
Process Optimization of Multiple Reaction Metrics. Chemistry‐Methods,
1(1), 71-77.

\bibitem{go_med1}Nour, M., Cömert, Z., \& Polat, K. (2020). A novel
medical diagnosis model for COVID-19 infection detection based on
deep features and Bayesian optimization. Applied Soft Computing, 97,
106580.
...
\bibitem{interval2}Wang, L., Chen, Z., Yang, G., Sun, Q., \& Ge,
J. (2020). An interval uncertain optimization method using back-propagation
neural network differentiation. Computer Methods in Applied Mechanics
and Engineering, 366, 113065.
...
\bibitem{genetic1}Chen, M., Yu, L., Zhi, C., Sun, R., Zhu, S., Gao,
Z., ... \& Zhang, Y. (2022). Improved faster R-CNN for fabric defect
detection based on Gabor filter with Genetic Algorithm optimization.
...
\bibitem{pso2}Shafiei Chafi, Z., \& Afrakhte, H. (2021). Short‐Term
Load Forecasting Using Neural Network and Particle Swarm Optimization
(PSO) Algorithm. Mathematical Problems in Engineering, 2021(1), 5598267.
...
\bibitem{WOA2}Zan, J. (2022). Research on robot path perception and
optimization technology based on whale optimization algorithm. Journal
of Computational and Cognitive Engineering, 1(4), 201-208.
...
\bibitem{key-8}Harada, T., \& Alba, E. (2020). Parallel genetic algorithms:
a useful survey. ACM Computing Surveys (CSUR), 53(4), 1-39.
...
\bibitem{gpu2}Skinderowicz, R. (2020). Implementing a GPU-based parallel
MAX--MIN Ant System. Future Generation Computer Systems, 106, 277-295.

\bibitem{gpu3}Schmitz, A., Burak, S., Miller, J., \& Müller, M. S.
(2024). Parallel pattern compiler for automatic global optimizations.
...
\bibitem{key-1}Shen, Y., Zhang, C., Gharehchopogh, F. S., \& Mirjalili,
S. (2023). An improved whale optimization algorithm based on multi-population
evolution for global optimization and engineering design problems.
Expert Systems with Applications, 215, 119269\textbf{.}
