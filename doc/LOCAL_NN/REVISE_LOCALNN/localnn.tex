...
mechanical applications \citep{nn_mech1} etc.
...
\end{equation}
The number of elements in the weight vector are calculated as: $n=(d+2)H$.\textbf{
}Of course, other activation functions may be used, such as the tanh
function defined as
\begin{equation}
\mbox{tanh}\left(x\right)=\frac{e^{2x}+1}{e^{2x}-1}
\end{equation}
with similar approximation capabilities. Also, Guarnieri et al proposed
the usage of an adaptive spline activation function for neural networks
\citep{activation_spline}. Furthermore, Ertuğrul proposed the trained
activation function in neural networks \citep{activation_trained}.
A systematic review of activation functions for Artificial Neural
Networks can be found in the paper of Rasamoelina et al \citep{activation_review}.
...
the Differential Evolution method \citep{nn_de}, etc. The first four
methods are local optimization methods, used in a series of research
papers but they can be easily trapped in local minima of the error
function of Equation \ref{eq:eq1}. On the other hand, methods like
Particle Swarm Optimization or Differential Evolution are considered
global optimization methods aiming to discover the global minimum
of such functions and as a consequence they can avoid local minima
of the error function. A survey of stochastic methods for training
neural networks can be found in the work of Zhang and Suganthan \citep{nn_survey}.
...
studied in the recent literature is the initialization of the parameters
...
Genetic algorithms have been extensively studied in the modern literature
for the training of artificial neural networks or for the efficient
creation of their structure. For example, the work of Arifovic et
al \citep{ga_nn1} used to select the optimal architecture of an artificial
neural network. Also, Leung et al proposed a novel genetic algorithm
\citep{ga_nn2} to adjust the parameters and the structure of neural
networks. Gao et al proposed an efficient genetic algorithm \citep{ga_nn3}
with a new diffusing operator for neural network training. Recently,
Ahmadizar et al combined genetic algorithm with grammatical evolution
for optimal training of neural networks \citep{ga_nn4}. Additionally,
Kobrunov and Priezzhev suggested a hybrid genetic algorithm \citep{ga_nn5}
for efficient neural network training. Although Genetic Algorithms
can satisfactorily train an artificial neural network, in many cases
they get trapped in local minimum of the training error and this results
in poor performance of the neural network when applied to the test
set. To improve the performance of genetic algorithms, it is proposed
to periodically apply a minimization technique to randomly selected
...
Genetic algorithms have been used in conjuction with Simulated Annealing
in a series of research papers in the recent literature, such as the
work of Yu et al that combines genetic algorithm with simulated annealing
for large scale system energy integration \citep{ga_sa1}. Also, Ganesh
and Punniyamoorthy used hybrid genetic algorithms for optimization
of continuous - time prediction planning \citep{ga_sa2}. Additionally,
Hwang and He suggested the usage of simulated annealing to improve
a genetic algorithm that was applied on engineering problems \citep{ga_sa3}.
Furthermore, Li and Wei applied a genetic algorithm that was enhanced
with a Simulated Annealing method on multi-reservoir systems \citep{ga_sa4}.
A method that combines a genetic algorithm with simulated annealing
was also used in Smart City problems recently \citep{ga_sa5}.
...
%
\end{enumerate}
The overall algorithm is also outlined graphically as a series of
steps in Figure \ref{fig:algorithm}.

\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.4]{flowchart}
\par\end{centering}
\caption{The overall proposed algorithm.\label{fig:algorithm}}
...
\hline 
$L_{I}$ & Number of generations for local search & 20\tabularnewline
\hline 
$L_{K}$ & Number of chromosomes in local search & 20\tabularnewline
\hline 
...
\hline 
$F$ & Range of changes in Simulated Annealing & 0.10\tabularnewline
\hline 
$N_{R}$ & Number of changes in Simulated Annealing & 20\tabularnewline
\hline 
\end{tabular}
\end{table}
...
to train a neural network with $H$ processing nodes. The method used
here is the BFGS variant of Powell \citep{Powell}.
...
nodes. In the current work the improved PSO method as suggested by
Charilogis and Tsoulos is used \citep{ipso}.
...
neural network with $H$ processing nodes. The genetic algorithm used
here is a variant proposed by Tsoulos \citep{doublepop}.
...
error as measured on the test set. The bold notation is used to identify
the method with the lowest average classification error.\label{tab:expClass}}
...
\hline 
\hline 
APPENDICITIS & \textbf{18.00\%} & 25.00\% & 24.40\% & 22.60\%\tabularnewline
\hline 
AUSTRALIAN & 38.13\% & 38.30\% & 36.64\% & \textbf{32.42\%}\tabularnewline
\hline 
BALANCE & 8.64\% & \textbf{7.97\%} & 8.36\% & 8.10\%\tabularnewline
\hline 
BANDS & 36.67\% & 36.61\% & 34.92\% & \textbf{34.53\%}\tabularnewline
\hline 
CIRCULAR & 6.08\% & \textbf{4.24\%} & 5.13\% & 4.35\%\tabularnewline
\hline 
CLEVELAND & 77.55\% & 62.31\% & 57.21\% & \textbf{42.62\%}\tabularnewline
\hline 
DERMATOLOGY & 52.92\% & 17.69\% & 16.60\% & \textbf{12.12\%}\tabularnewline
\hline 
ECOLI & 69.52\% & 61.30\% & 54.67\% & \textbf{47.18\%}\tabularnewline
\hline 
FERT & \textbf{23.20\%} & 24.00\% & 28.50\% & 25.20\%\tabularnewline
\hline 
HEART & 39.44\% & 34.67\% & 26.41\% & \textbf{16.59\%}\tabularnewline
\hline 
HEARTATTACK & 46.67\% & 37.83\% & 29.03\% & \textbf{20.13\%}\tabularnewline
\hline 
HOUSEVOTES & 7.13\% & 7.87\% & \textbf{7.00\%} & 7.13\%\tabularnewline
\hline 
LIVERDISORDER & 42.59\% & 39.82\% & 37.09\% & \textbf{32.88\%}\tabularnewline
\hline 
PARKINSONS & 27.58\% & 23.58\% & \textbf{16.58\%} & 16.63\%\tabularnewline
\hline 
PIMA & 35.59\% & 35.17\% & 34.21\% & \textbf{30.08\%}\tabularnewline
\hline 
POPFAILURES & 5.24\% & 7.80\% & \textbf{4.17\%} & 5.44\%\tabularnewline
\hline 
REGIONS2 & 36.28\% & 31.43\% & 33.53\% & \textbf{27.69\%}\tabularnewline
\hline 
SAHEART & 37.48\% & 34.80\% & 34.85\% & \textbf{34.56\%}\tabularnewline
\hline 
SEGMENT & 68.97\% & 53.88\% & 46.30\% & \textbf{28.41\%}\tabularnewline
\hline 
SONAR & 25.85\% & 24.70\% & 22.40\% & \textbf{19.80\%}\tabularnewline
\hline 
SPIRAL & 47.99\% & 46.31\% & 47.67\% & \textbf{44.54\%}\tabularnewline
\hline 
WDBC & 29.91\% & 9.98\% & 7.87\% & \textbf{5.66\%}\tabularnewline
\hline 
WINE & 59.71\% & 32.71\% & 22.88\% & \textbf{10.59\%}\tabularnewline
\hline 
Z\_F\_S & 39.37\% & 38.73\% & 24.60\% & \textbf{11.10\%}\tabularnewline
\hline 
ZO\_NF\_S & 43.04\% & 30.38\% & 21.54\% & \textbf{6.86\%}\tabularnewline
\hline 
ZONF\_S & 15.62\% & 6.92\% & 4.36\% & \textbf{2.48\%}\tabularnewline
\hline 
ZOO & 10.70\% & 9.20\% & 9.50\% & \textbf{7.60\%}\tabularnewline
\hline 
...
error as measure on the test set. The bold notation is used to express
the method with the lowest average regression error. \label{tab:expRegression}}
\begin{tabular}{|c|c|c|c|c|}
\hline 
\textbf{DATASET} & \textbf{BFGS} & \textbf{PSO} & \textbf{GENETIC} & \textbf{PROPOSED}\tabularnewline
\hline 
\hline 
AIRFOIL & 0.003 & 0.001 & 0.001 & \textbf{0.001}\tabularnewline
\hline 
BK & 0.36 & 0.33 & 0.26 & \textbf{0.18}\tabularnewline
\hline 
BL & 1.09 & 2.49 & 2.23 & \textbf{0.42}\tabularnewline
\hline 
BASEBALL & 119.63 & 82.81 & 64.60 & \textbf{57.47}\tabularnewline
\hline 
DEE & 2.36 & 0.43 & 0.47 & \textbf{0.23}\tabularnewline
\hline 
EU & 607.61 & 407.35 & 252.97 & \textbf{216.65}\tabularnewline
\hline 
FY & 0.19 & \textbf{0.05} & 0.65 & 0.23\tabularnewline
\hline 
HO & 0.62 & \textbf{0.03} & 0.37 & 0.06\tabularnewline
\hline 
HOUSING & 97.38 & 43.28 & 35.97 & \textbf{23.77}\tabularnewline
\hline 
LW & 0.26 & \textbf{0.03} & 0.54 & 0.27\tabularnewline
\hline 
MORTGAGE & 8.23 & 1.47 & 0.40 & \textbf{0.05}\tabularnewline
\hline 
MUNDIAL & \textbf{0.05} & 0.08 & 1.22 & 0.28\tabularnewline
\hline 
PL & 0.11 & 0.06 & 0.03 & \textbf{0.02}\tabularnewline
\hline 
QUAKE & 0.09 & 0.06 & 0.12 & \textbf{0.06}\tabularnewline
\hline 
REALESTATE & 128.94 & 81.41 & 81.19 & \textbf{72.95}\tabularnewline
\hline 
SN & 0.16 & 0.40 & 0.20 & \textbf{0.05}\tabularnewline
\hline 
TREASURY & 9.91 & 2.32 & 0.44 & \textbf{0.26}\tabularnewline
\hline 
VE & 1.92 & \textbf{0.32} & 2.43 & 1.63\tabularnewline
\hline 
...
datasets is shown in Figure \ref{fig:statClass}.
...
Nevertheless, the proposed method significantly may increase the total
execution time and this can be observed from the graph of Figure \ref{fig:timeComparison}.

\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.4]{siman_time}
\par\end{centering}
\caption{Time comparison of the average execution time for the classification
datasets.\label{fig:timeComparison}}

\end{figure}
The proposed technique significantly improves the performance of the
genetic algorithm in minimizing the training error of the artificial
neural networks but requires significantly more computing time. However,
the required computing time could be significantly reduced either
by using parallel computing techniques.
...
$F$. The experiments were executed on the classification datasets.
The numbers in cells denote average classification error, as measured
on the test set.\label{tab:experF}}
...
the used classification datasets. The numbers in cells stand for the
average classification error, as measured on the corresponding test
set.\label{tab:experNR}}
...
for various values of the parameter $N_{I}$. Numbers in cells denote
average classification error as measured on the test set.\label{tab:experNI}}
...
optimal values of the error function.\textbf{ }The main contributions
of the current work are:
\begin{enumerate}
\item Periodic application of an intelligent stochastic technique based
on Simulated Annealing. This technique improves the training error
of randomly selected chromosomes.
\item By using parameters, the changes that this stochastic method can cause
in the chromosomes are controlled.
\item This stochastic technique can be used without modification in both
classification and data fitting problems.
\end{enumerate}
The new training method is quite general and has been successfully
applied to a variety of data classification and data fitting problems.
This new technique significantly improves the performance of Genetic
Algorithms in almost all data sets that were used, and in fact, in
several of them the reduction in the error can reach up to 80\%. The
proposed method achieved a significant reduction in error compared
to all the techniques with which it was compared. This reduction starts
on average from 20\% for the case of genetic algorithms and ends in
a reduction of 45\% for the case of the BFGS optimization method.
Furthermore, the technique's behavior and performance are not significantly
affected by any variations in its critical parameters except for the
$F$ parameter, which controls the magnitude of changes that can be
made to a chromosome. However, the effect of this parameter seems
to decrease for large values. 
...
\bibitem{ga_nn3}Q. Gao, K. Qi, Y. Lei, Z. He, An Improved Genetic
Algorithm and Its Application in Artificial Neural Network Training,
In: 2005 5th International Conference on Information Communications
\& Signal Processing, Bangkok, pp. 357-360, 2005.

\bibitem{ga_nn4}F. Ahmadizar, K. Soltanian, F. AkhlaghianTab, I.
Tsoulos, Artificial neural network development by means of a novel
...
\bibitem{ga_nn5}A. Kobrunov, I. Priezzhev, Hybrid combination genetic
algorithm and controlled gradient method to train a neural network,
Geophysics \textbf{81}, pp. 35-43, 2016.
...
\bibitem{ga_sa1}H. Yu, H. Fang, P. Yao, Y. Yuan, A combined genetic
algorithm/simulated annealing algorithm for large scale system energy
integration, Computers \& Chemical Engineering \textbf{24}, pp. 2023-2035,
2000. 

\bibitem{ga_sa2}K. Ganesh, M. Punniyamoorthy, Optimization of continuous-time
production planning using hybrid genetic algorithms-simulated annealing,
Int J Adv Manuf Technol \textbf{26}, pp. 148--154, 2005.

\bibitem{ga_sa3}S.-F. Hwang, R.-S. He, Improving real-parameter genetic
algorithm with simulated annealing for engineering problems, Advances
in Engineering Software \textbf{37}, pp. 406-418, 2006. 

\bibitem{ga_sa4}X.G. Li, X. Wei, An Improved Genetic Algorithm-Simulated
Annealing Hybrid Algorithm for the Optimization of Multiple Reservoirs,
Water Resour Manage \textbf{22}, pp. 1031--1049, 2008.

\bibitem{ga_sa5}P. Suanpang, P. Jamjuntr, K. Jermsittiparsert, P.
Kaewyong, Tourism Service Scheduling in Smart City Based on Hybrid
Genetic Algorithm Simulated Annealing Algorithm, Sustainability \textbf{14},
16293, 2022.
