...
TRIO::TRIO()
{
    ...
    addParam(Parameter("population", 240, 10, 2000000, "Population size"));
...
void TRIO::probeLandscape_()
{
    ...
    // --- Ruggedness via short random-walk & lag-1 autocorrelation of diffs ---
    int steps = std::max(4, std::min(probe_steps, budget/2));
    double hrel = 0.02; // μικρό σχετικό βήμα
    ...
    double hax = 0.02; // σχετική διαταραχή
    double axis_sum = 0.0;
    for (int j : dims){
    ...
    // προσοχή στο budget — κόψαμε ήδη όπου χρειάζεται
}

void TRIO::applyLandscapeProfile_()
{
    // Τρία προφίλ: Aggressive (rugged), Exploit (separable & smooth), Balanced
    // thresholds:
...
// Μία extra αξιολόγηση: γραμμική προβολή στον άξονα trial−base (λ∈{1,0.5})
...
void TRIO::step()
{
    ...
    for (int i=s; i<e; ++i){
        ...
        // 1-step refine πάνω στη γραμμή x→bestT
        vector<double> Tref;
