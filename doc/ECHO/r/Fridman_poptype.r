...
# Εισαγωγή δεδομένων
dat <- read_excel("C:/Users/admin/Desktop/ECHO/poptype.xlsx")

# Μετασχηματισμός δεδομένων σε μακρύ format
df <- dat %>%
  pivot_longer(cols = c(CLPSO, jDE, SaDE, "CMA-ES", PROPOSED),
               names_to = "Method",
               values_to = "Error") %>%
  mutate(
    FUNCTION = as.factor(FUNCTION),
    Method = factor(Method, levels = c("CLPSO", "jDE", "SaDE", "CMA-ES", "PROPOSED"))
  )
...
p <- ggboxplot(df, x = "Method", y = "Error", color = "Method",
  ...
  annotate("text", x = 3.0, y = y_max * 2.4,
  ...
  annotate("text", x = 0.9, y = y_max * 2.4,
