...
# Εισαγωγή δεδομένων
dat <- read_excel("C:/Users/admin/Desktop/ECHO/class.xlsx")

# Μετασχηματισμός δεδομένων σε μακρύ format
df <- dat %>%
  pivot_longer(cols = c(GENETIC, ECHO, "ECHO+APP", "ECHO+MEM", "ECHO+APP+MEM"),
               names_to = "Method",
               values_to = "Error") %>%
  mutate(
    DATASET = as.factor(DATASET),
    Method = factor(Method, levels = c("GENETIC", "ECHO", "ECHO+APP", "ECHO+MEM", "ECHO+APP+MEM"))
  )

# 1. Friedman Test
friedman_result <- friedman_test(Error ~ Method | DATASET, data = df)

# 2. Υπολογισμός Κρίσιμης Διαφοράς (CD)
k <- nlevels(df$Method)      # Αριθμός μεθόδων (ομάδων)
N <- nlevels(df$DATASET)    # Αριθμός blocks (συναρτήσεων)
