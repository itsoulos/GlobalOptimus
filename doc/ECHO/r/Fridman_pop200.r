...
# Εισαγωγή δεδομένων
dat <- read_excel("C:/Users/admin/Desktop/ECHO/pop200.xlsx")

# Μετασχηματισμός δεδομένων σε μακρύ format
df <- dat %>%
  pivot_longer(cols = c(GA, PSO, SaDE, jDE, "CMA-ES", PROPOSED),
               names_to = "Method",
               values_to = "Error") %>%
  mutate(
    FUNCTION = as.factor(FUNCTION),
    Method = factor(Method, levels = c("GA", "PSO", "SaDE", "jDE", "CMA-ES", "PROPOSED"))
  )
...
p <- ggboxplot(df, x = "Method", y = "Error", color = "Method",
  ...
  annotate("text", x = 3.8, y = y_max * 3.0,
  ...
  annotate("text", x = 0.9, y = y_max * 3.0,
