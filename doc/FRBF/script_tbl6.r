...
get_pair_p <- function(a, b, P) {
  ...
  if (!is.null(rns) && !is.null(cns)) {
    if (a %in% rns && b %in% cns && !is.na(P[a, b])) return(P[a, b])
    if (b %in% rns && a %in% cns && !is.na(P[b, a])) return(P[b, a])
  }
  return(NA_real_)
}
...
p <- ggplot(common_df, aes(x = Method, y = Error, color = Method)) +
  ...
  annotate(
    "text",
    x = max(1, length(present_methods) - 2),
    y = 80,
