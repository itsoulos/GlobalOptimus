...
a function:
\begin{equation}
R\left(\overrightarrow{x}\right)=\sum^{k}_{i=1}w_{i}\phi\left(\left\Vert \overrightarrow{x}-\overrightarrow{c_{i}}\right\Vert \right)\label{eq:firstrbf}
\end{equation}
...
\par\end{centering}
\caption{General structure of an RBF network. The input vector $\protect\overrightarrow{x}$
is fed to $k$ hidden units, each characterized by a center $\protect\overrightarrow{c_{i}}$
(a reference point in the input space) and a variance $\sigma_{i}$
(the spread of the basis function). The output $R(x)$ is a weighted
sum of the hidden unit responses, with weights $w_{i}$. }

\end{figure}

The following notation is used in Equation \ref{eq:firstrbf}:
\begin{enumerate}
\item The vector $\overrightarrow{x}=(x_{1},x_{2},...,x_{d})$ denotes the
input pattern.
...
RBF network. The vector $\overrightarrow{w}$ contains the coefficients
used to weight the output of each basis function $\phi(x)$.
\end{enumerate}
Moreover, the training error associated with any RBF network $R(x)$
is defined as:
\begin{equation}
E\left(R(x)\right)=\sum^{M}_{i=1}\left(R\left(\overrightarrow{x_{i}}\right)-y_{i}\right)^{2}\label{eq:RbfError}
\end{equation}
...
wind energy production prediction \citep{rbf_wind}, constrained optimization
\citep{rbfcons1,rbfcons2} etc.
...
Moreover, Mohammadi et al proposed \citep{rbf_hybrid1} a novel hybrid
evolutionary based RBF networks method for forecasting time series
that can estimate both the network architecture as well as the values
for the parameters of the network. Likewise, Wu et al introduced \citep{rbf_hybrid2}
a hybrid method that combines the Particle Swarm Optimization (PSO)
\citep{pso_tutorial} method as well as Genetic Algorithms to determine
the parameters of RBF networks. Recently, Elansari proposed a new
method \citep{rbf_hybrid3} that utilizes Genetic Algorithms for the
optimal training of RBF networks. Likewise, Mukherjee et al developed
hybrid Gaussian Radial Basis Neural Networks (GRAB-NNs) for data-driven
models in a recent study \citep{rbf_hybrid4}. Beyond genetic algorithms,
other population-based metaheuristics have also been applied to RBF
training, including differential evolution {[}\citep{Bajer}{]} and
particle swarm optimization {[}\citep{Alexandridis,Rani}{]}.

A third research direction recognizes that purely global metaheuristics,
while effective at exploration, converge slowly in the vicinity of
the optimum, motivating hybrid or memetic algorithms that combine
global search with local refinement {[}\citep{Moscato,Ong}{]}. Recent
RBF-specific examples include hybrid schemes combining particle swarm
optimization, cuckoo search, and biogeography-based optimization,
refined by a local BFGS step {[}\citep{Ali}{]}.
...
The present work is positioned at the intersection of all three research
directions described above: the first phase draws on the initialization
literature by using K-means to establish informed, data-driven search
bounds rather than fixed or arbitrary ranges; the second phase contributes
a genetic algorithm operating within these bounds, following the evolutionary
training paradigm; and the local optimization step applied at the
end of the second phase follows the hybrid/memetic paradigm. Unlike
prior evolutionary RBF methods that treat the search space as unconstrained,
and unlike prior hybrid methods that do not exploit data-driven initialization
to constrain the search space before the global search begins, the
core contribution of the proposed method lies in this three-stage
integration.

The main contributions of the proposed method are summarized as follows:
\begin{enumerate}
\item The k-means clustering algorithm is employed to effectively identify
suitable value ranges for the parameters of the RBF network.
\item A well-established global optimization technique, namely Genetic Algorithms
(GAs), is introduced to efficiently determine the optimal parameters
of an RBF network, regardless of the dataset to which it is applied.
\item A numerical method is proposed for estimating the output weights of
the RBF network.
\item A local optimization procedure is incorporated to further reduce the
training error of the RBF network.
\end{enumerate}
...
presented in Algorithm \ref{alg:KMEANS}. This algorithm is an unsupervised
clustering technique that splits a dataset into a number of predefined
clusters by minimizing the within-cluster sum of squared distances.
The algorithm iteratively assigns each data point to its nearest cluster
centroid and at every generation updates the centroid as the mean
of the assigned points. This process is repeated until convergence.
...
\begin{enumerate}
\item \textbf{Input}: the training set $\left(\overrightarrow{x_{i}},y_{i}\right),\ i=1,...,M$
of the objective problem.
\item \textbf{Set} as $k$ the number of desired centers.
\item \textbf{Assign }randomly every pattern $x_{i},\ i=1,...,M$ in a cluster
$S_{j},\ j=1,\ldots,k$.
\item \textbf{For} each cluster $j=1..k$ \textbf{do}
\begin{enumerate}
\item \textbf{Denote }with $M_{j}$ the number of points in $S_{j}$
\item \textbf{Estimate }every center $c_{j}$ as
\[
\overrightarrow{c_{i}}=\frac{1}{M_{j}}\sum_{\overrightarrow{x_{i}}\in S_{j}}\overrightarrow{x_{i}}
\]
\end{enumerate}
\item \textbf{End For}
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Set} $j^{*}=\mbox{argmin}^{k}_{m=1}\left\{ D\left(x_{i},c_{m}\right)\right\} $.
...
\end{enumerate}
\item \textbf{End For}
\item \textbf{For} $j=1,\ldots,k$ \textbf{do}
\begin{enumerate}
...
\overrightarrow{c_{i}}=\frac{1}{M_{j}}\sum_{\overrightarrow{x_{i}}\in S_{j}}\overrightarrow{x_{i}}
\]
\item \textbf{Calculate} the quantities $\sigma_{j}$ as 
\[
\sigma^{2}_{j}=\frac{\sum^{M_{j}}_{i=1}\left(x_{i}-c_{j}\right)^{2}}{M_{j}}
\]
\end{enumerate}
...
as $F$ with $F\ge1$.
\item \textbf{Input}: a small positive number $v_{e}$ used as the lower
limit of the variables representing the variances of the RBF network.
\item \textbf{Set} $m=1$
...
of each center
\begin{enumerate}
\item \textbf{Set} $L_{m}=-F\times\left|I_{m}\right|$ //this denotes the
$m$ element of vector $\overrightarrow{L}$. The value $I_{m}$ represents
the $m$ element of vector $\overrightarrow{I}$.
\item \textbf{Set} $R_{m}=F\times\left|I_{m}\right|$ //this denotes the
$m$ element of vector $\overrightarrow{R}$.
\item \textbf{Set} $m=m+1$ 
\end{enumerate}
\item \textbf{End For}
\item \textbf{Set} $L_{m}=v_{e}$ //set the bounds for the variables representing
the variances.
\item \textbf{Set} $R_{m}=F\times\left|I_{m}\right|$
\item \textbf{Set} $m=m+1$
\end{enumerate}
\item \textbf{End For}
...
of practical applications. Genetic Algorithms (GAs) are evolutionary
optimization techniques designed to search for the global minimum
of a multidimensional objective function. To achieve this, a GA maintains
a population of candidate solutions, referred to as chromosomes. These
chromosomes evolve continuously over successive iterations of the
algorithm, known as generations, through the application of genetic
operators that mimic natural evolutionary processes, including selection,
crossover, and mutation. In addition, they are particularly well suited
for integration with machine learning models, as they generally require
only the formulation of a suitable fitness function while treating
the underlying model as a black-box optimization problem. For example
they have been applied to energy problems \citep{gen_app1}, water
distribution problems \citep{gen_app2}, economic problems \citep{gen_app3},
training of of neural networks \citep{gen_app4}, statistics \citep{gen_statistics},
RBF interpolation \citep{gen_inter} etc. The steps of the used genetic
...
\item \textbf{Set} $\mbox{it}=0$, which represents the current number of
generations.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Produce }an RBF network $R_{i}=R\left(\overrightarrow{x}\right).$
...
\begin{enumerate}
\item \textbf{Set} \textbf{$W=\left[w_{1},w_{2},\ldots,w_{k}\right]$}
\item \textbf{Set} \textbf{$\Phi=\phi_{j}\left(x_{i}\right)$}, that denotes
the outputs of the computation units.
\item \textbf{Set $T=\left\{ t_{j}=y_{j},j=1,..,M\right\} $}, that denotes
...
\begin{equation}
f_{i}=\sum^{M}_{j=1}\left(R_{i}\left(\overrightarrow{x}_{j}\right)-y_{j}\right)^{2}\label{eq:eq1-1}
\end{equation}
\end{enumerate}
\item \textbf{End For}
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Set} $\mbox{it}=\mbox{it}+1$
\item \textbf{If} $\mbox{it}<N_{g}$then \textbf{goto} Fitness Calculation
step.
\item \textbf{Otherwise, }obtain the best chromosome $\overrightarrow{g^{*}}$
and create the associated RBF network $R^{*}=R\left(\overrightarrow{x}\right)$.
...
\begin{equation}
f^{*}=\sum^{M}_{j=1}\left(R^{*}\left(\overrightarrow{x}_{j}\right)-y_{j}\right)^{2}\label{eq:eq1-1-1}
\end{equation}
...
The datasets used in the conducted experiments were obtained from
...
electricity. 
\item The\textbf{ FA} dataset, which is related to fat measurements. 
...
automatic differentiation library \citep{adept}. All the experiments
were conducted on\textbf{ }an AMD Ryzen 5950X with 128GB of RAM and
the used operating system was Debian Linux. During the experimental
evaluation, the 10-fold cross-validation technique was employed, where
each dataset was partitioned into ten folds. In each iteration, 90\%
of the instances were used as the training set, while the remaining
10\% constituted the test set. Both the training procedure and the
application of the k-means initialization method were performed exclusively
on the training set, with no information from the test set being used
during model construction. Each experimental setup was repeated 30
independent times, with a distinct initialization seed assigned to
the random number generator in every run. Random number generation
was carried out using the drand48() function provided by the C programming
language. The average classification error is defined as: 
\begin{equation}
...
\end{equation}
The set $T$ denotes the corresponding test set, with $T=\left(x_{i},y_{i}\right),\ i=1,\ldots,M$.
Also, the average regression error is defined as follows:
\begin{equation}
E_{R}\left(N\left(\overrightarrow{x},\overrightarrow{w}\right)\right)=\frac{\sum^{N}_{i=1}\left(N\left(\overrightarrow{x_{i}},\overrightarrow{w}\right)-y_{i}\right)^{2}}{M}
\end{equation}
...
nodes.
\item The column RBFCON denotes the incorporation of a recently introduced
method \citep{rbfcon} that constructs RBF networks with the assistance
of Grammatical Evolution \citep{mainGE}.
...
for NEAT, 28.68\% for RBF, 24.96\% for RBFCON, and 21.58\% for the
Proposed method. The overall Friedman test (Figure \ref{fig:classMethods})
yielded $p=3.60\times10^{-16}$, confirming that the differences among
methods are statistically significant at a very high confidence level.
The Proposed method achieves the lowest mean error and outperforms
the second-best method, RBFCON, by approximately 13.5\%. At the individual
dataset level, the Proposed method ranked first in 21 out of 42 datasets
(50.0\%), with particularly strong performance on RING, SPAMBASE,
Z\_F\_S, and ZONF\_S, as detailed in Table \ref{tab:experClass}.
...
\hline 
DATASET & ADAM & BFGS & NEAT & RBF & GRBF & RBFCON & PROPOSED\tabularnewline
\hline 
\hline 
APPENDICITIS & 16.50\% & 18.00\% & 17.20\% & 12.23\% & 16.83\% & 13.60\% & 17.80\%\tabularnewline
\hline 
ALCOHOL & 57.78\% & 41.50\% & 66.80\% & 49.38\% & 52.45\% & 51.24\% & 20.43\%\tabularnewline
\hline 
AUSTRALIAN & 35.65\% & 38.13\% & 31.98\% & 34.89\% & 41.79\% & 14.14\% & 18.90\%\tabularnewline
\hline 
BALANCE & 12.27\% & 8.64\% & 23.14\% & 33.53\% & 38.02\% & 39.94\% & 17.50\%\tabularnewline
\hline 
BANDS & 36.92\% & 36.67\% & 34.30\% & 37.17\% & 43.95\% & 35.75\% & 36.28\%\tabularnewline
\hline 
CIRCULAR & 19.95\% & 6.08\% & 35.18\% & 5.98\% & 21.43\% & 18.26\% & 5.04\%\tabularnewline
\hline 
CLEVELAND & 67.55\% & 77.55\% & 53.44\% & 67.10\% & 67.47\% & 49.14\% & 50.14\%\tabularnewline
\hline 
DERMATOLOGY & 26.14\% & 52.92\% & 32.43\% & 62.34\% & 61.46\% & 45.20\% & 39.80\%\tabularnewline
\hline 
ECOLI & 64.43\% & 69.52\% & 43.44\% & 59.48\% & 70.58\% & 54.18\% & 58.36\%\tabularnewline
\hline 
FERT & 23.98\% & 23.20\% & 15.37\% & 15.00\% & 15.00\% & 15.20\% & 21.40\%\tabularnewline
\hline 
GLASS & 61.38\% & 54.67\% & 55.71\% & 50.05\% & 66.91\% & 55.38\% & 52.76\%\tabularnewline
\hline 
HABERMAN & 29.00\% & 29.34\% & 24.04\% & 25.10\% & 24.37\% & 26.27\% & 27.37\%\tabularnewline
\hline 
HAYES ROTH & 59.70\% & 37.33\% & 50.15\% & 64.36\% & 63.46\% & 34.54\% & 40.08\%\tabularnewline
\hline 
HEART & 38.53\% & 39.44\% & 39.27\% & 31.20\% & 28.44\% & 17.22\% & 22.00\%\tabularnewline
\hline 
HEARTATTACK & 45.55\% & 46.67\% & 32.34\% & 29.00\% & 40.48\% & 22.60\% & 19.13\%\tabularnewline
\hline 
HOUSEVOTES & 7.48\% & 7.13\% & 10.89\% & 6.13\% & 11.99\% & 3.05\% & 4.61\%\tabularnewline
\hline 
IONOSPHERE & 16.64\% & 15.29\% & 19.67\% & 16.22\% & 19.83\% & 14.32\% & 12.83\%\tabularnewline
\hline 
LIVERDISORDER & 41.53\% & 42.59\% & 30.67\% & 30.84\% & 36.97\% & 32.21\% & 30.41\%\tabularnewline
\hline 
LYMOGRAPHY & 39.79\% & 35.43\% & 33.70\% & 25.50\% & 29.33\% & 26.36\% & 24.57\%\tabularnewline
\hline 
MAMMOGRAPHIC & 46.25\% & 17.24\% & 22.85\% & 21.38\% & 30.41\% & 17.67\% & 17.72\%\tabularnewline
\hline 
PARKINSONS & 24.06\% & 27.58\% & 18.56\% & 17.41\% & 33.81\% & 12.79\% & 15.53\%\tabularnewline
\hline 
PAGE BLOCKS & 34.27\% & 8.49\% & 10.22\% & 10.09\% & 10.83\% & 11.09\% & 9.47\%\tabularnewline
\hline 
PHONEME & 29.43\% & 15.58\% & 22.34\% & 23.32\% & 27.70\% & 24.58\% & 15.59\%\tabularnewline
\hline 
PIMA & 34.85\% & 35.59\% & 34.51\% & 25.78\% & 27.83\% & 24.13\% & 23.59\%\tabularnewline
\hline 
POPFAILURES & 5.18\% & 5.24\% & 7.05\% & 7.04\% & 7.08\% & 6.98\% & 5.61\%\tabularnewline
\hline 
REGIONS2 & 29.85\% & 36.28\% & 33.23\% & 38.29\% & 39.98\% & 26.34\% & 25.71\%\tabularnewline
\hline 
RING & 28.80\% & 29.44\% & 30.85\% & 21.67\% & 52.03\% & 11.08\% & 5.05\%\tabularnewline
\hline 
SAHEART & 34.04\% & 37.48\% & 34.51\% & 32.19\% & 33.90\% & 29.52\% & 28.89\%\tabularnewline
\hline 
SATIMAGE & 73.22\% & 71.73\% & 76.19\% & 40.19\% & 61.20\% & 59.16\% & 32.97\%\tabularnewline
\hline 
SEGMENT & 49.75\% & 68.97\% & 66.72\% & 59.68\% & 54.25\% & 62.36\% & 32.47\%\tabularnewline
\hline 
SONAR & 30.33\% & 25.85\% & 34.10\% & 27.90\% & 37.13\% & 27.20\% & 24.00\%\tabularnewline
\hline 
SPAMBASE & 48.05\% & 18.16\% & 35.77\% & 29.35\% & 35.36\% & 16.95\% & 10.54\%\tabularnewline
\hline 
SPIRAL & 47.67\% & 47.99\% & 48.66\% & 44.87\% & 50.02\% & 42.14\% & 41.03\%\tabularnewline
\hline 
STATHEART & 44.04\% & 39.65\% & 44.36\% & 31.36\% & 42.94\% & 19.22\% & 23.89\%\tabularnewline
\hline 
STUDENT & 5.13\% & 7.14\% & 10.20\% & 5.49\% & 33.26\% & 7.63\% & 12.43\%\tabularnewline
\hline 
TRANSFUSION & 25.68\% & 25.84\% & 24.87\% & 26.41\% & 25.67\% & 25.15\% & 25.23\%\tabularnewline
\hline 
WDBC & 35.35\% & 29.91\% & 12.88\% & 7.27\% & 8.82\% & 6.77\% & 4.19\%\tabularnewline
\hline 
WINE & 29.40\% & 59.71\% & 25.43\% & 31.41\% & 31.47\% & 11.00\% & 10.59\%\tabularnewline
\hline 
Z\_F\_S & 47.81\% & 39.37\% & 38.41\% & 13.16\% & 23.37\% & 11.13\% & 4.68\%\tabularnewline
\hline 
ZO\_NF\_S & 47.43\% & 43.04\% & 43.75\% & 9.02\% & 22.18\% & 11.08\% & 5.47\%\tabularnewline
\hline 
ZONF\_S & 11.99\% & 15.62\% & 5.44\% & 4.03\% & 17.41\% & 4.14\% & 1.82\%\tabularnewline
\hline 
ZOO & 14.13\% & 10.70\% & 20.27\% & 21.93\% & 33.50\% & 11.60\% & 10.60\%\tabularnewline
\hline 
\textbf{AVERAGE} & \textbf{35.18\%} & \textbf{33.25\%} & \textbf{32.16\%} & \textbf{28.68\%} & \textbf{35.50\%} & \textbf{24.96\%} & \textbf{21.58\%}\tabularnewline
\hline 
\end{tabular}
\end{table}
...
(overall Friedman test: $p=3.60\times10^{-16}$)\label{fig:classMethods}}
\end{figure}

\begin{table}[H]
\caption{Mean classification error and average Friedman rank across the 42
datasets (lower rank = better). Overall Friedman $\chi^{2}=84.82,p=3.60\times10^{-16}$.\label{tab:rank1}}

\begin{centering}
\begin{tabular}{|c|c|c|}
\hline 
\textbf{Method} & \textbf{Mean Error} & \textbf{Avg Rank}\tabularnewline
\hline 
\hline 
PROPOSED & 21.58 & 2.02\tabularnewline
\hline 
RBFCON & 24.96 & 2.74\tabularnewline
\hline 
RBF & 28.68 & 3.61\tabularnewline
\hline 
NEAT & 32.16 & 4.55\tabularnewline
\hline 
BFGS & 33.25 & 4.64\tabularnewline
\hline 
ADAM & 35.18 & 5.02\tabularnewline
\hline 
GRBF & 35.50 & 5.42\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
\end{table}

Post-hoc pairwise comparisons (Conover test, Holm-corrected) confirm
that the Proposed method differs significantly (p\_adj \textless{}
0.05) from RBF (p\_adj = 0.0276), NEAT (p\_adj = $5.39\times10^{-6}$),
BFGS (p\_adj = $2.31\times10^{-6}$), ADAM (p\_adj = $2.06\times10^{-8}$),
and GRBF (p\_adj = $7.76\times10^{-11}$), but does not differ significantly
from RBFCON (p\_adj = 0.7358), indicating statistically comparable
classification performance between the two methods.
...
\hline 
\hline 
ABALONE & 4.30 & 5.69 & 9.88 & 7.37 & 9.98 & 6.14 & 6.37\tabularnewline
\hline 
AIRFOIL & 0.005 & 0.003 & 0.067 & 0.27 & 0.121 & 0.004 & 0.004\tabularnewline
\hline 
AUTO & 70.84 & 60.97 & 56.06 & 17.87 & 16.78 & 11.03 & 10.02\tabularnewline
\hline 
BK & 0.025 & 0.28 & 0.15 & 0.02 & 0.023 & 0.02 & 0.021\tabularnewline
\hline 
BL & 0.622 & 2.55 & 0.05 & 0.013 & 0.005 & 0.04 & 0.0005\tabularnewline
\hline 
BASEBALL & 77.90 & 119.63 & 100.39 & 93.02 & 98.91 & 66.74 & 53.20\tabularnewline
\hline 
CONCRETE & 0.078 & 0.066 & 0.081 & 0.011 & 0.015 & 0.012 & 0.008\tabularnewline
\hline 
DEE & 0.63 & 2.36 & 1.512 & 0.17 & 0.25 & 0.23 & 0.16\tabularnewline
\hline 
FA & 0.048 & 0.426 & 0.19 & 0.015 & 0.15 & 0.013 & 0.019\tabularnewline
\hline 
FY & 0.038 & 0.22 & 0.08 & 0.011 & 0.041 & 0.017 & 0.051\tabularnewline
\hline 
HO & 0.035 & 0.62 & 0.169 & 0.03 & 0.076 & 0.013 & 0.007\tabularnewline
\hline 
HOUSING & 80.998 & 97.38 & 56.49 & 57.68 & 95.69 & 20.6 & 10.36\tabularnewline
\hline 
LASER & 0.03 & 0.015 & 0.084 & 0.03 & 0.075 & 0.06 & 0.0198\tabularnewline
\hline 
LW & 0.028 & 2.98 & 0.17 & 0.03 & 0.057 & 0.011 & 0.016\tabularnewline
\hline 
MB & 0.06 & 0.129 & 0.061 & 5.43 & 0.159 & 0.055 & 0.066\tabularnewline
\hline 
MORTGAGE & 9.24 & 8.23 & 14.11 & 1.45 & 1.92 & 0.165 & 0.0096\tabularnewline
\hline 
NT & 0.006 & 0.129 & 0.027 & 13.97 & 0.01 & 0.006 & 0.006\tabularnewline
\hline 
PLASTIC & 11.71 & 20.32 & 20.77 & 8.62 & 25.91 & 3.58 & 2.30\tabularnewline
\hline 
PY & 0.321 & 0.578 & 0.075 & 0.012 & 0.029 & 0.003 & 0.015\tabularnewline
\hline 
PL & 0.117 & 0.290 & 0.098 & 2.118 & 0.155 & 0.064 & 0.024\tabularnewline
\hline 
QUAKE & 0.039 & 0.012 & 0.102 & 0.01 & 0.79 & 0.036 & 0.036\tabularnewline
\hline 
SN & 0.026 & 0.400 & 0.174 & 0.027 & 0.027 & 0.025 & 0.024\tabularnewline
\hline 
STOCK & 180.893 & 302.43 & 215.818 & 12.23 & 25.18 & 8.31 & 1.208\tabularnewline
\hline 
TREASURY & 11.16 & 9.91 & 15.52 & 2.02 & 1.89 & 0.003 & 0.047\tabularnewline
\hline 
VE & 0.359 & 1.92 & 0.045 & 0.024 & 0.023 & 0.04 & 0.04\tabularnewline
\hline 
\textbf{AVERAGE} & \textbf{17.98} & \textbf{25.50} & \textbf{19.69} & \textbf{8.90} & \textbf{11.13} & \textbf{4.69} & \textbf{3.36}\tabularnewline
\hline 
\end{tabular}
\end{table}
...
test: $p=1.26\times10^{-12}$).\label{fig:regressionMethods}}
\end{figure}

\begin{table}[H]
\caption{Mean regression error (raw scale) and average Friedman rank across
the 25 datasets. The average rank is scale-free and is not dominated
by high-magnitude datasets (e.g., STOCK, BASEBALL), unlike the raw
mean error. Overall Friedman $\chi^{2}=67.61,p=1.26\times10^{-12}$\label{tab:rank2}}
...
\hline 
\hline 
PROPOSED & 3.36 & 2.06\tabularnewline
\hline 
RBFCON & 4.69 & 2.24\tabularnewline
\hline 
RBF & 8.90 & 3.58\tabularnewline
\hline 
ADAM & 17.98 & 4.34\tabularnewline
\hline 
GRBF & 11.13 & 4.62\tabularnewline
\hline 
NEAT & 19.69 & 5.56\tabularnewline
\hline 
BFGS & 25.50 & 5.60\tabularnewline
\hline 
\end{tabular}
\end{table}

\textit{The raw average regression error is disproportionately influenced
by datasets with large target-value magnitudes (e.g., STOCK, BASEBALL);
the scale-free average rank in Table \ref{tab:rank2} confirms the
same overall ordering, with the Proposed method achieving the best
(lowest) average rank of 2.06 across the 25 datasets.}

Post-hoc pairwise comparisons confirm that the Proposed method differs
significantly from ADAM (p\_adj = 0.0108), GRBF (p\_adj = 0.0022),
NEAT (p\_adj = $1.06\times10^{-6}$), and BFGS (p\_adj = $8.65\times10^{-7}$)
(all p\_adj \textless{} 0.05), but the differences from RBFCON (p\_adj
= 0.9999) and RBF (p\_adj = 0.3272) did not reach statistical significance,
indicating that its advantage over these two closest competitors is
directionally consistent but not conclusively established at the individual-comparison
level.
...
8.90 for RBF, 11.13 for GRBF, 4.69 for RBFCON, and 3.36 for the Proposed
method. The overall Friedman test (Figure \ref{fig:regressionMethods})
yielded p =$1.26\times10{{}^-}{{}^1}{{}^2}$ , again confirming statistically
significant differences across methods. The Proposed method achieves
a mean error of 3.36, representing an improvement of about 28\% over
RBFCON (4.69), nearly threefold improvement over RBF (8.90), and nearly
eightfold (not tenfold) improvement over BFGS (25.50).
...
$p=6.43\times10^{-5}$, confirming that the choice of $F$ has a statistically
...
yielding $p=3.09\times10^{-4}$. A consistent improvement is again
...
Proposed method (overall Friedman test: $p=6.43\times10^{-5}$)\label{fig:classF}.}
\end{figure}
...
Friedman test: $p=3.09\times10^{-4}$).\label{fig:regressionF}}
\end{figure}
...
yielded p = 0.0010, confirming a statistically significant effect
...
method (overall Friedman test: $p=0.0010$).\label{fig:classNc}}
\end{figure}
...
\end{figure}
Furthermore, the proposed method was compared with the GRBF method
on the classification datasets for a range of population sizes (i.e.,
different numbers of chromosomes). The comparison was performed with
respect to both the average classification error and the execution
time. The comparison for the average classification error is presented
in Figure \ref{fig:classErrorNc} and the comparison for the execution
time is depicted in Figure \ref{fig:classTimeNc}.

\begin{figure}[H]
\includegraphics[scale=0.9]{frbf_class_error_nc}

\caption{Comparison of average classification error between the proposed method
and the GRBF method on the series of selected classification datasets
for different values of used chromosomes.\label{fig:classErrorNc}}

\end{figure}
\begin{figure}[H]
\includegraphics[scale=0.9]{frbf_class_time_nc}

\caption{Comparison of average execution time between the proposed method and
the GRBF method on the series of classification datasets, for different
values of number of chromosomes.\label{fig:classTimeNc}}

\end{figure}
The proposed method significantly outperforms the GRBF technique in
terms of average classification error. Moreover, the classification
error remains essentially unchanged for population sizes exceeding
500 chromosomes, supporting the choice of this value for the parameter
$N_{c}$. However, the execution time of the proposed method is considerably
higher than that of the GRBF method, particularly for large population
sizes. This increase is expected and can be attributed to the greater
computational complexity of the proposed approach compared with the
GRBF method.

\subsection{Experiments with the major parts of the method}

An additional set of ablation experiments was conducted by removing
key components of the proposed methodology to assess its robustness
and quantify the contribution of each component to the overall performance.
In the first experiment, the local optimization procedure was removed,
and the experiments were carried out on both the classification and
regression datasets for different values of the critical parameter
$F$. The average classification error for all classification datasets
is depicted in Figure \ref{fig:classLocal}. Similarly, figure \ref{fig:regressionLocal}
presents the average regression error obtained by the application
of the proposed method to the used regression datasets. The column
LOCAL denoted the incorporation of the BFGS optimizer in the proposed
method and the column NOLOCAL represents the application of the proposed
method without the BFGS optimizer.

\begin{figure}[H]
\includegraphics[scale=0.75]{frbf_class_local}

\caption{Average classification error of the proposed method for the series
of classification datasets using different values for parameter $F$
with and without local optimization.\label{fig:classLocal}}

\end{figure}
\begin{figure}[H]
\includegraphics[scale=0.75]{frbf_regression_local}

\caption{Average regression error of the proposed method for the regression
datasets using different values for parameter $F$ with and without
local optimization.\label{fig:regressionLocal}}

\end{figure}
The above plots clearly demonstrate that the incorporation of the
local optimization procedure improves the performance of the proposed
technique, with a more pronounced improvement observed on the regression
datasets than on the classification datasets.

In the next experiment, an attempt was made to evaluate the effectiveness
of the k-means algorithm for estimating the initial range of values
of the RBF network parameters. Figure \ref{fig:classInit} presents
the average classification error using the proposed method and different
initialization ranges and Figure \ref{fig:regresionInit} stands for
the average regression error using the proposed method and the same
initialization ranges. The columns presented in graphs have the following
meaning:
\begin{enumerate}
\item The column INIT=1 denotes the application of the proposed method,
where the variables of the RBF network are initialized in range $[-1,1]$.
\item The column INIT=10 represents the application of the proposed method
and initialization for the variables in range $[-10,10]$.
\item The column INIT=20 stands for the incorporation of the proposed method
and initialization for the variables in range $[-20,20]$.
\item The column PROPOSED represents the usage of the proposed method with
the initialization presented in this article with the k-means technique. 
\end{enumerate}
\begin{figure}[H]
\includegraphics[scale=0.75]{frbf_class_init}

\caption{Average classification error obtained in the used classification datasets
using the proposed method and different initialization ranges.\label{fig:classInit}}

\end{figure}
\begin{figure}[H]
\includegraphics[scale=0.75]{frbf_regression_init}

\caption{Average classification error obtained in the used regression datasets
with the application of the proposed method and different initialization
ranges.\label{fig:regresionInit}}

\end{figure}
It is evident that, in both cases (classification and regression),
the proposed initialization technique significantly improves the performance
of the proposed method.
...
While the Proposed method achieved the lowest error on roughly half
of the benchmark datasets (21 of 42 classification and 13 of 25 regression
datasets), it was outperformed by at least one baseline on the remaining
cases, and these cases follow identifiable patterns. For classification,
RBFCON emerged as the most competitive alternative, achieving the
lowest error on 7 datasets (AUSTRALIAN, CLEVELAND, HAYES ROTH, HEART,
HOUSEVOTES, PARKINSONS, and STATHEART), a cluster of predominantly
medium-sized, binary or near-binary medical and economic classification
problems. This is consistent with the post-hoc analysis (Table \ref{tab:rank1}),
which shows that the performance gap between the Proposed method and
RBFCON does not reach statistical significance (p\_adj = 0.7358),
unlike the gaps with all other baselines. Beyond RBFCON, the remaining
losses largely follow the previously observed pattern of small sample
size (e.g., APPENDICITIS, FERT, HABERMAN, STUDENT, POPFAILURES) or
a relatively large number of classes with limited samples per class
(e.g., DERMATOLOGY, ECOLI, GLASS, BANDS, BALANCE, TRANSFUSION), where
a fixed number of hidden units (k=10) and limited training data may
not provide enough signal for the evolutionary search to consistently
outperform simpler, directly-optimized alternatives. Notably, in several
of these cases (e.g., MAMMOGRAPHIC, PAGE BLOCKS, PHONEME), the performance
gap with the best-performing baseline was under 1 percentage point
and is unlikely to be practically meaningful. For regression, RBFCON
was again the strongest competing method, prevailing on FA, LW, MB,
PY, and TREASURY, while RBF remained competitive on BK, FY, QUAKE,
and VE; correspondingly, the post-hoc tests (Table \ref{tab:rank2})
show that the Proposed method's advantage over both RBFCON (p\_adj
= 0.9999) and RBF (p\_adj = 0.3272) does not reach statistical significance,
whereas its advantage over ADAM, BFGS, NEAT, and GRBF is significant
(p\_adj \textless{} 0.05 in all four cases). These datasets are generally
characterized by low target-value magnitude and low intrinsic noise,
where traditional RBF training, grammar-guided architecture search,
or direct gradient-based optimization already achieve a near-optimal
fit; in such cases, the broader exploration performed by the genetic
algorithm offers limited additional benefit and may introduce marginal
variance. Taken together, these observations suggest that the advantage
of the proposed evolutionary training is most pronounced on larger,
higher-dimensional, or structurally complex datasets, while on small-sample,
low-noise, or already well-fit problems, RBFCON and, to a lesser extent,
conventional RBF training may already be near-sufficient.

A limitation of the present study is that only the mean error across
the 30 independent runs was retained for each dataset--method combination;
per-run standard deviations were not stored during the original experiments
and could therefore not be reported. Future revisions of the experimental
protocol will retain per-run results to support variability analysis
...
\bibitem{rbfde1}Bhatia, G. S., \& Arora, G. (2016). Radial basis
function methods for solving partial differential equations-a review.
Indian Journal of Science and Technology, 9(45), 1-18.

\bibitem{rbfde2}Cavoretto, R., \& De Rossi, A. (2020). A two-stage
adaptive scheme based on RBF collocation for solving elliptic PDEs.
Computers \& Mathematics with Applications, 79(11), 3206-3222.

\bibitem{rbfde3}Feng, R., \& Duan, J. (2018). High accurate finite
differences based on RBF interpolation and its application in solving
differential equations. Journal of Scientific Computing, 76(3), 1785-1812.

\bibitem{rbfrobotics1}Song, Q., Li, S., Bai, Q., Yang, J., Zhang,
A., Zhang, X., \& Zhe, L. (2021). Trajectory planning of robot manipulator
based on RBF neural network. Entropy, 23(9), 1207.

\bibitem{rbfrobotics2}Liu, Q., Li, D., Ge, S. S., Ji, R., Ouyang,
Z., \& Tee, K. P. (2021). Adaptive bias RBF neural network control
for a robotic manipulator. Neurocomputing, 447, 213-223.

\bibitem{rbfface1}Roh, S. B., Oh, S. K., Yoon, J. H., \& Seo, K.
(2019). Design of face recognition system based on fuzzy transform
and radial basis function neural networks. Soft Computing, 23(13),
4969-4985.

\bibitem{rbfnetwork1}Lopez-Martin, M., Sanchez-Esguevillas, A., Arribas,
J. I., \& Carro, B. (2021). Network intrusion detection based on extended
RBF neural network with offline reinforcement learning. IEEE Access,
9, 153153-153170.

\bibitem{rbfnetwork2}Chen, Z. (2022). Research on internet security
situation awareness prediction technology based on improved RBF neural
network algorithm. Journal of Computational and Cognitive Engineering,
1(3), 103-108.

\bibitem{rbfchemistry1}Tao, J., Yu, Z., Zhang, R., \& Gao, F. (2021).
RBF neural network modeling approach using PCA based LM--GA optimization
for coke furnace system. Applied Soft Computing, 111, 107691.

\bibitem{rbfchemistry2}Kanti, P., Sharma, K. V., Yashawantha, K.
M., Jamei, M., \& Said, Z. (2022). Properties of water-based fly ash-copper
hybrid nanofluid for solar energy applications: Application of RBF
model. Solar Energy Materials and Solar Cells, 234, 111423.

\bibitem{rbfecon0}Yu, Y. (2022). GDP economic forecasting model based
on improved RBF neural network. Mathematical Problems in Engineering,
2022(1), 7630268.

\bibitem{rbfecon1}Zhang, Q., Abdullah, A. R., Chong, C. W., \& Ali,
M. H. (2022). A Study on Regional GDP Forecasting Analysis Based on
Radial Basis Function Neural Network with Genetic Algorithm (RBFNN‐GA)
for Shandong Economy. Computational Intelligence and Neuroscience,
2022(1), 8235308.
...
\bibitem{rbf_dos1}Anyanwu, G. O., Nwakanma, C. I., Lee, J. M., \&
Kim, D. S. (2022). Optimization of RBF-SVM kernel using grid search
algorithm for DDoS attack detection in SDN-based VANET. IEEE Internet
of Things Journal, 10(10), 8477-8490.
...
\bibitem{rbfcons1}Regis, R. G. (2014). Constrained optimization by
radial basis function interpolation for high-dimensional expensive
black-box problems with infeasible initial points. Engineering Optimization,
46(2), 218-243.

\bibitem{rbfcons2}Wu, Y., Yin, Q., Jie, H., Wang, B., \& Zhao, J.
(2018). A RBF-based constrained global optimization algorithm for
problems with computationally expensive objective and constraints.
Structural and Multidisciplinary Optimization, 58(4), 1633-1655.
...
\bibitem{rbf_hybrid1}Mohammadi, R., Ghomi, S. F., \& Zeinali, F.
(2014). A new hybrid evolutionary based RBF networks method for forecasting
time series: a case study of forecasting emergency supply demand time
series. Engineering Applications of Artificial Intelligence, 36, 204-214.

\bibitem{rbf_hybrid2}Wu, J., Long, J., \& Liu, M. (2015). Evolving
RBF neural networks for rainfall prediction using hybrid particle
swarm optimization and genetic algorithm. Neurocomputing, 148, 136-142.

\bibitem{pso_tutorial}Marini, F., \& Walczak, B. (2015). Particle
swarm optimization (PSO). A tutorial. Chemometrics and intelligent
laboratory systems, 149, 153-165.

\bibitem{rbf_hybrid3}Elansari, T., Ouanan, M., \& Bourray, H. (2023).
Mixed radial basis function neural network training using genetic
algorithm. Neural Processing Letters, 55(8), 10569-10587.

\bibitem{rbf_hybrid4}Mukherjee, A., Gupta, D., \& Bhattacharyya,
D. (2025). Mass-Constrained hybrid Gaussian radial basis neural networks:
Development, training, and applications to modeling nonlinear dynamic
noisy chemical processes. Computers \& Chemical Engineering, 197,
109080.

\bibitem{Bajer}D. Bajer, B. Zorić, G. Martinović, Automatic design
of radial basis function networks through enhanced differential evolution,
in: Hybrid Artificial Intelligent Systems, Springer, pp. 244-256,
2015.

\bibitem{Alexandridis}A. Alexandridis, E. Chondrodima, H. Sarimveis,
Cooperative learning for radial basis function networks using particle
swarm optimization, Applied Soft Computing 49, pp. 485-497, 2016.

\bibitem{Rani}H.J. Rani, T.A.A. Victoire, Training radial basis function
networks for wind speed prediction using PSO enhanced differential
search optimizer, PLOS ONE 13(5), e0196871, 2018.

\bibitem{Moscato}P. Moscato, On evolution, search, optimization,
genetic algorithms and martial arts: Towards memetic algorithms, Caltech
Concurrent Computation Program, Technical Report 826, 1989.

\bibitem{Ong}Y.S. Ong, A.J. Keane, Meta-Lamarckian learning in memetic
algorithms, IEEE Transactions on Evolutionary Computation 8(2), pp.
99-110, 2004.

\bibitem{Ali}S. Ali, J.A. Kurz, S. Oughton, A Novel RBF Neural Network-based
Hybrid Technique and Its Applications, Research Square preprint, doi:10.21203/rs.3.rs-7088954/v1,
2025.
...
\bibitem{gen_statistics}S. Chatterjee, M. Laudato, et al. Genetic
algorithms and their statistical appli-cations: an introduction. Comput.
Statist. Data Anal., 22 (6), 633--651 (1996). \url{https://doi.org/10.1016/0167-9473(96)00011-4.}

\bibitem{gen_inter}R. Cavoretto, A. De Rossi, et al. Genetic Algorithm-Based
Shape Parameter Tuning for Radial Basis Function Interpolation. Oper.
Res. Forum 7, 26 (2026).\url{ https://doi.org/10.1007/s43069-026-00610-9}
...
\bibitem{rbfcon}Tsoulos, I. G., Varvaras, I., \& Charilogis, V. (2024).
RbfCon: construct radial basis function neural networks with grammatical
evolution. Software, 3(4), 549-568.

\bibitem{mainGE}Ryan, C., O'Neill, M., \& Collins, J. J. (Eds.).
(2018). Handbook of grammatical evolution (Vol. 1). Cham (Switzerland):
