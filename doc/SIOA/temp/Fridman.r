...
# Εισαγωγή δεδομένων
dat <- read_excel("C:/Users/admin/Desktop/SIOA/pop100.xlsx")

# Μετασχηματισμός δεδομένων σε μακρύ format
df <- dat %>%
  pivot_longer(cols = c(SIOA, GA, DE, PSO, PSO, ACO),
               names_to = "Method",
               values_to = "Error") %>%
  mutate(
    FUNCTION = as.factor(FUNCTION),
    Method = factor(Method, levels = c("SIOA", "GA", "DE", "PSO", "ACO"))
  )
...
posthoc <- df %>%
  ...
  mutate(
    ...
    significance_group = case_when(
      group2 == "SIOA" & is_significant == "Yes" ~ "Superior to SIOA",
      group1 == "SIOA" & is_significant == "Yes" ~ "Superior to SIOA",
      TRUE ~ "Not different"
    )
  )
...
p <- ggboxplot(df, x = "Method", y = "Error", color = "Method",
  ...
  annotate("text", x = 3.2, y = y_max * 2.5,
           label = "ns: p > 0.05 (Not significant)\n**: p < 0.01 (Highly significant\n****: p < 0.0001  (Very extremely significant)",
           hjust = 0, size = 2.8, color = "blue") +
  annotate("text", x = 0.9, y = y_max * 2.5,
