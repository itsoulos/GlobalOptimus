...
methods, Swarm based methods, termination technique, sampling method}
...
\begin{itemize}
\item The addition of a sampling technique based on the K-means method {[}47,
48{]}. The sampling points will facilitate finding the global minimum
of the function in the most efficient way. Additionally, by applying
...
step, \textbf{else} terminate and return the best solution.\caption{EGO Algorithm}
\end{enumerate}
\end{algorithm}
...
definitions of these functions are given below in Table .
\begin{table}[H]
\centering{}\caption{The test functions used in the conducted experiments.\label{tab:testFunctions}}
\begin{tabular}{lcc}
\toprule 
Test function & Formula & Dimension\tabularnewline
\midrule
\midrule 
Bohachevsky1 & {\tiny{}$f(x)=x_{1}^{2}+2x_{2}^{2}-\frac{3}{10}\cos\left(3\pi x_{1}\right)-\frac{4}{10}\cos\left(4\pi x_{2}\right)+\frac{7}{10}$} & 2\tabularnewline
\midrule 
Bohachevsky2 & {\tiny{}$f(x)=x_{1}^{2}+2x_{2}^{2}-\frac{3}{10}\cos\left(3\pi x_{1}\right)\cos\left(4\pi x_{2}\right)+\frac{3}{10}$} & 2\tabularnewline
\midrule 
Bohachevsky3 & {\tiny{}$f(x)=x_{1}^{2}+2x_{2}^{2}-\frac{3}{10}\cos\left(3\pi x_{1}+4\pi x_{2}\right)+\frac{3}{10}$} & 2\tabularnewline
\midrule 
Branin & {\tiny{}$f(x)=\left(x_{2}-\frac{5.1}{4\pi^{2}}x_{1}^{2}+\frac{5}{\pi}x_{1}-6\right)^{2}+10\left(1-\frac{1}{8\pi}\right)\cos(x_{1})+10$} & 2\tabularnewline
\midrule 
Camel & {\tiny{}$f(x)=4x_{1}^{2}-2.1x_{1}^{4}+\frac{1}{3}x_{1}^{6}+x_{1}x_{2}-4x_{2}^{2}+4x_{2}^{4},\quad x\in[-5,5]^{2}$} & 2\tabularnewline
\midrule 
Easom & {\tiny{}$f(x)=-\cos\left(x_{1}\right)\cos\left(x_{2}\right)\exp\left(\left(x_{2}-\pi\right)^{2}-\left(x_{1}-\pi\right)^{2}\right)$} & 2\tabularnewline
\midrule 
Exponential & {\tiny{}$f(x)=-\exp\left(-0.5\sum_{i=1}^{n}x_{i}^{2}\right),\quad-1\le x_{i}\le1$} & {\scriptsize{}$n=4,8,16,32$}\tabularnewline
\midrule 
Griewank2 & {\tiny{}$f(x)=1+\frac{1}{200}\sum_{i=1}^{2}x_{i}^{2}-\prod_{i=1}^{2}\frac{\cos(x_{i})}{\sqrt{(i)}},\quad x\in[-100,100]^{2}$} & 2\tabularnewline
\midrule 
Griewank10 & {\tiny{}$f(x)=\sum_{i=1}^{n}\frac{x_{i}^{2}}{4000}-\prod_{i=1}^{n}\cos\left(\frac{x_{i}}{\sqrt{i}}\right)+1$} & 10\tabularnewline
\midrule 
Gkls & {\tiny{}$f(x)=\mbox{Gkls}(x,n,w)$} & {\scriptsize{}$n=2,3\ w=50,100$}\tabularnewline
\midrule 
Hansen & {\tiny{}$f(x)=\sum_{i=1}^{5}i\cos\left[(i-1)x_{1}+i\right]\sum_{j=1}^{5}j\cos\left[(j+1)x_{2}+j\right]$} & 2\tabularnewline
\midrule 
Hartman3 & {\tiny{}$f(x)=-\sum_{i=1}^{4}c_{i}\exp\left(-\sum_{j=1}^{3}a_{ij}\left(x_{j}-p_{ij}\right)^{2}\right)$} & 3\tabularnewline
\midrule 
Hartman6 & {\tiny{}$f(x)=-\sum_{i=1}^{4}c_{i}\exp\left(-\sum_{j=1}^{6}a_{ij}\left(x_{j}-p_{ij}\right)^{2}\right)$} & 6\tabularnewline
\midrule 
Potential & {\tiny{}$V_{LJ}(r)=4\epsilon\left[\left(\frac{\sigma}{r}\right)^{12}-\left(\frac{\sigma}{r}\right)^{6}\right]$} & {\scriptsize{}$n=3,5$}\tabularnewline
\midrule 
Rastrigin & {\tiny{}$f(x)=x_{1}^{2}+x_{2}^{2}-\cos(18x_{1})-\cos(18x_{2}),\quad x\in[-1,1]^{2}$} & 2\tabularnewline
\midrule 
Rosenbrock & {\tiny{}$f(x)=\sum_{i=1}^{n-1}\left(100\left(x_{i+1}-x_{i}^{2}\right)^{2}+\left(x_{i}-1\right)^{2}\right),\quad-30\le x_{i}\le30$} & {\scriptsize{}$n=4,8,16$}\tabularnewline
\midrule 
Shekel5 & {\tiny{}$f(x)=-\sum_{i=1}^{5}\frac{1}{(x-a_{i})(x-a_{i})^{T}+c_{i}}$} & 4\tabularnewline
\midrule 
Shekel7 & {\tiny{}$f(x)=-\sum_{i=1}^{7}\frac{1}{(x-a_{i})(x-a_{i})^{T}+c_{i}}$} & 4\tabularnewline
\midrule 
Shekel10 & {\tiny{}$f(x)=-\sum_{i=1}^{10}\frac{1}{(x-a_{i})(x-a_{i})^{T}+c_{i}}$} & 4\tabularnewline
\midrule 
Sinusoidal & {\tiny{}$f(x)=-\left(2.5\prod_{i=1}^{n}\sin\left(x_{i}-z\right)+\prod_{i=1}^{n}\sin\left(5\left(x_{i}-z\right)\right)\right),\quad0\le x_{i}\le\pi$} & {\scriptsize{}$n=4,8$}\tabularnewline
\midrule 
Test2n & {\tiny{}$f(x)=\frac{1}{2}\sum_{i=1}^{n}x_{i}^{4}-16x_{i}^{2}+5x_{i},\quad x_{i}\in[-5,5]$} & {\scriptsize{}$n=3,4,5,6,7$}\tabularnewline
\midrule 
Test30n & {\tiny{}$f(x)=\frac{1}{10}\sin^{2}\left(3\pi x_{1}\right)\sum_{i=2}^{n-1}\left(\left(x_{i}-1\right)^{2}\left(1+\sin^{2}\left(3\pi x_{i+1}\right)\right)\right)+\left(x_{n}-1\right)^{2}\left(1+\sin^{2}\left(2\pi x_{n}\right)\right)$} & {\scriptsize{}$n=3,4$}\tabularnewline
\bottomrule
\end{tabular}
\end{table}
