...
proposed a one-to-one mapping for chromosomes and the non - terminal
symbols of the grammar. Also, O'Neill et al. suggested the usage of
shape grammars in the Grammatical Evolution for evolutionary design
\citep{ge_shape}. Also the $\pi$Grammatical Evolution method \citep{pge}
was suggested as a modification of the Grammatical Evolution, where
...
\citep{pso1} to create expressions in Grammatical Evolution. This
method was named Grammatical Swarm \citep{ge_swarm1,ge_swarm2} in
the relevant literature. Moreover, the method of Probabilistic Grammatical
Evolution \citep{probge} has been introduced recently, where a stochastic
procedure was used as a mapping mechanism. Recently, the optimization
method of Fireworks algorithm \citep{fireworks} was applied as a
learning algorithm for the Grammatical Evolution procedure \citep{ge_fireworks}.
Contreras et al. suggested the combination of Grammatical Evolution
and some ideas from interval analysis, to solve problems with uncertainty
\citep{ge_interval}. Grammatical evolution has also been extended
using programming techniques \citep{ge_par1,ge_par2} or Christiansen
grammars \citep{ge_christiansen}.\textbf{ }
...
\begin{figure}[H]
\caption{The BNF grammar used in the rule construction method.\label{fig:ruleGrammar}}
...
\textless boolexpr\textgreater ::=\textless expr\textgreater ~\textless rop\textgreater ~\textless expr\textgreater ~(0)

~~~~~~~~~~~~\textbar\textless boolexpr\textgreater ~\textless bop\textgreater ~\textless boolexpr\textgreater ~(1)
...
the sequence number of each rule. The parameter $d$ stands for the
dimension of the used dataset ( number of features). Grammatical Evolution
produces valid expressions by starting from the starting symbol $S$
and by following the production rules. The method selects the following
production rules according to the scheme:
\begin{itemize}
\item \textbf{Get} the next element $V$ for the current chromosome.
...
neural networks using the following general form:
...
\item \textbf{EndFor}
\end{enumerate}
\item \textbf{Application of genetic operations step.}
\begin{enumerate}
...
offsprings are produced from the current population. For every set
$\left(\tilde{z},\tilde{w}\right)$ of produced children, two distinct
chromosomes $(z,w)$ are selected from the population with tournament
selection. The offsprings $\left(\tilde{z},\tilde{w}\right)$ are
formulated using the one - point crossover procedure, graphically
outlined in Figure \ref{fig:onePoint}.
\item \textbf{Perform} the mutation procedure. A random number $r\in[0,1]$
is selected for each element of every chromosome and this element
is changed randomly if $r\le p_{m}$.
...
on $g_{i}$.
\end{enumerate}
\item \textbf{Check for termination step}. 
\begin{enumerate}
...
on a selected chromosome using a set of randomly chosen chromosomes
...
A series of classification datasets was used in the conducted experiments:
\begin{enumerate}
...
\item \textbf{Fert }dataset, which is used to detect any relation of fertility
...
about breast cancer.
\item \textbf{Parkinsons} dataset, related to the detection of Parkinson's
...
\item \textbf{Regions2} dataset, a dataset used in the detection of hepatitis
C \citep{regions}. 
\item \textbf{Saheart} dataset \citep{saheart}, involved to the detection
of heart diseases.
\item \textbf{Segment} dataset \citep{segment}, which contains information
regarding image processing.
\item \textbf{Spiral} dataset, that is an artificial dataset.
...
various experiments regarding EEG \citep{eeg}. The following cases
were used from this dataset: Z\_F\_S, Z\_O\_N\_F\_S, ZO\_NF\_S and
ZONF\_S.
\item \textbf{Zoo} dataset \citep{zoo}, used to estimate the category of
some animals.
\end{enumerate}

\subsection{Regression datasets }

A series of regression datasets was utilized in the conducted experiments:
\begin{enumerate}
...
\item \textbf{Dee} dataset, that contains measurements regarding the price
of electricity.
...
of babies.
\item \textbf{MORTGAGE} dataset, that contains economic measurements from
USA.
...
\item \textbf{REALESTATE, }originated in the STALIB repository.
\item \textbf{SN} dataset, that is used in an experiment related to trellising
and pruning. 
\item \textbf{Treasury} dataset, which is a dataset regarding the economy
of USA.
\item \textbf{TZ} dataset, founded in the STALIB repository.
\item \textbf{VE} dataset, originated in the STALIB repository.
\end{enumerate}

\subsection{Experimental results}

The code used in the experiments was coded in ANSI C++ using the Optimus
optimization environment, available from \url{https://github.com/itsoulos/GlobalOptimus/}(
...
regression error as measured on the test set was recorded. In order
to validate the experimental results, the well -known method of ten
- fold cross validation was incorporated.\textbf{ }Table \ref{tab:values}
depicts the experimental settings for the performed experiments.

\begin{table}[H]
\caption{The values for the experimental parameters. These parameters were
used in the machine learning methods.\label{tab:values}}
...
\end{tabular}
\end{table}
The experimental results obtained for the case of classification are
depicted in Table \ref{tab:experClass} and the experimental results
for the case of regression datasets are displayed in Table \ref{tab:experRegression}.
The following notations are used in the experimental tables:
\begin{enumerate}
...
\item The average classification or regression error is depicted in row
...
the regression datasets are depicted in Figures \ref{fig:boxRuleRegression}
...
\begin{table}[H]
\caption{The effect of different values of $p_{cr}$ to the RULE model with
...
neural network construction method was tested on the series of classification
...
in required execution time can be significantly reduced by using modern
parallel libraries. These programming techniques may include the application
of the Message Passing Interface (MPI) library \citep{mpi} or the
incorporation of the OpenMP library \citep{openmp}.
...
\end{itemize}
The methods were tested on a wide series of datasets in the recent
literature. In the vast majority of cases, the application of the
new genetic operator resulted in a drastic reduction of the corresponding
...
more experiments were conducted in which these critical parameters
were changed over a wide range of values. Boosting these values improves
the performance of machine learning methods by applying the new genetic
