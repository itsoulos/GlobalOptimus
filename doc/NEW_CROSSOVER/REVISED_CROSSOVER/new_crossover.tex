...
This method was used in a series of cases derived from real - world
problems, such as\textbf{ }data fitting \citep{ge_program1,ge_program2},
...
energy problems \citep{ge_energy}, combinatorial optimization \citep{ge_comb},\textbf{
...
proposed a one-to-one mapping for chromosomes and the set of non -
terminal symbols. Also, O'Neill et al. suggested the usage of shape
grammars in the Grammatical Evolution for evolutionary design \citep{ge_shape}.
Also the $\pi$Grammatical Evolution method \citep{pge} was suggested
...
the QFc software \citep{ge_qfc} the used to create new features from
the initial features with the assistance of Grammatical Evolution
etc.
...
used datasets are described followed by the experimental results and
in section \ref{sec:Conclusions} some conclusions are discussed in
detail.
...
\begin{itemize}
\item \textbf{Get} the next element $V$ for the processed chromosome.
\item \textbf{Select} the rule that will be applied as:\textbf{ }
\begin{equation}
...
by the Grammatical Evolution may be the following:
\[
...
\end{equation}
The vector $\overrightarrow{x}$ represents the input vector, while
the vector $\overrightarrow{p}$ stands for the parameter set of the
neural network. The parameter $H$ represents the number of weights
or processing units. These networks have one processing level, but
the grammar could be easily extended to produce neural networks of
additional levels. The function $\mbox{sig}(x)$ used in the previous
example denotes the sigmoid function given by:
\begin{equation}
...
\item \textbf{Set} as $N_{cr}$ the amount of chromosomes the will be selected
...
\par\end{centering}
\caption{A flowchart indicating the main steps of the used genetic algorithm.\label{fig:mainGa}}
...
randomly from the current population. The steps of this procedure
are listed below:
\begin{enumerate}
...
A series of datasets from this category was utilized in the experiments:
\begin{enumerate}
...
\end{tabular}
\end{table}
Table \ref{tab:experClass} outlines the experimental results for
the classification datasets while Table \ref{tab:experRegression}
hold the results for the mentioned regression datasets. The following
