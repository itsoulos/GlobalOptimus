...
\abstract{The presented work outlines a new genetic crossover operator, which
can be used to problems solved by the Grammatical Evolution technique.
...
their fitness value. The new operator is applied to chromosomes selected
randomly from the under processing genetic population. This new operator
was applied to two techniques from the recent literature that exploit
Grammatical Evolution: artificial neural network construction and
rule construction. In both case studies, an extensive set of classification
problems and data fitting problems were incorporated to estimate the
effectiveness of the proposed genetic operator. The proposed operator
significantly reduced both the classification error on the classification
datasets and the feature learning error on the fitting datasets, compared
to other machine learning techniques and also to the original models
...
and mutation \citep{gen1,gen2,gen3}. The genetic algorithms were
incorporated in a variety of problems, such as networking problems\textbf{
}\citep{ga_problem1},\textbf{ }problems arise in robotics \citep{ga_problem2,ga_problem3},
energy problems \citep{ga_problem4,ga_problem5}, medicine problems
\citep{Doewes,Choudhury}, agriculture problems \citep{Chen} etc.
...
Evolution can be utilized to produce programs in the provided programming
language. This method was used in a series of cases derived from real
...
}computer games \citep{ge_pacman,ge_supermario}, problems regarding
energy demand \citep{ge_energy}, combinatorial optimization \citep{ge_comb},\textbf{
}security topics \citep{ge_crypt},\textbf{ }automatic construction
of decision trees \citep{ge_decision}, circuit design \citep{ge_analog},
discovering taxonomies in Wikipedia \citep{ge_wikipedia}, trading
algorithms \citep{ge_trading}, bioinformatics \citep{ge_bio}, modeling
glycemia in humans \citep{ge_glykemia}, etc.
...
proposed a novel technique to map genotypes to phenotypes, or the
method of Structured Grammatical Evolution \citep{ge_structured1,ge_structured2}.
Also, O'Neill et al. suggested the usage of shape grammars in the
Grammatical Evolution for evolutionary design \citep{ge_shape}. Also
the $\pi$Grammatical Evolution method \citep{pge} was suggested
as a modification of the Grammatical Evolution, where a position -
independent mapping was proposed. Another interesting work was the
incorporation of the Particle Swarm Optimization(PSO) \citep{pso1}
to create expressions in Grammatical Evolution. This method was named
Grammatical Swarm \citep{ge_swarm1,ge_swarm2} in the relevant literature.
Moreover, the method of Probabilistic Grammatical Evolution \citep{probge}
has been introduced recently, where a stochastic procedure was used
as a mapping mechanism. Recently, the optimization method of Fireworks
algorithm \citep{fireworks} was applied as a learning algorithm for
the Grammatical Evolution procedure \citep{ge_fireworks}. Contreras
et al. suggested the combination of Grammatical Evolution and some
ideas from interval analysis, to solve problems with uncertainty \citep{ge_interval}.
...
an R implementation of Grammatical Evolution under the name gramEvol\textbf{
}\citep{ge_gramevol}\textbf{, }the GRAPE software that implemented
Grammatical Evolution in Python \citep{ge_grape},\textbf{ }the GeLab
\citep{ge_gelab} software that suggested a Matlab toolbox for Grammatical
Evolution, a software which produced classification programs with
Grammatical Evolution called GenClass \citep{ge_genclass}, the QFc
software \citep{ge_qfc} was incorporated for feature construction
etc.

In this work, a new genetic operator for Grammatical Evolution is
introduced, which is derived from the one crossover technique. The
new genetic operator is stochastically applied to the genetic population,
...
tools were applied on a some classification and regression datasets
proposed in the recent literature and the experimental results indicated
a reduction in classification or regression error from the application
of the new genetic operator.
...
\item The new genetic operator could theoretically be applied to other forms
of genetic algorithms beyond Grammatical Evolution. 
\end{enumerate}
The following sections have this structure: in section \ref{sec:Materials-and-Methods}
the basic principles of Grammatical Evolution are discussed as well
as the current genetic operator. In section \ref{sec:Results} the
used datasets and the experimental results are presented and in section
\ref{sec:Conclusions} some conclusions are discussed in detail.

\section{Materials and Methods\label{sec:Materials-and-Methods}}

\subsection{Basics of the Grammatical Evolution method}

The Grammatical Evolution considers chromosomes as set of production
rules in the provided BNF grammar. BNF grammar is usually denoted
as a set $G=\left(N,T,S,P\right)$, with the following assumptions:
\begin{itemize}
\item $N$ defines the set of non-terminal symbols.
\item $T$ is a set that contains the terminal symbols. 
\item $S$ is a non - terminal symbol that corresponds to the that start
symbol of the grammar.
\item $P$ is the set of the rules used during production. These rules are
in the form $A\rightarrow a$ or $A\rightarrow aB,\ A,B\in N,\ a\in T$.
In Grammatical Evolution procedure a sequence number is assigned to
every production rule.
\end{itemize}
The grammar used for the rule machine learning method is outlined
in Figure \ref{fig:ruleGrammar} and the corresponding for the neural
network construction technique is displayed in figure \ref{fig:grammarNNC}.

\begin{figure}[H]
\caption{The BNF grammar used in the method that constructs rules using the
Grammatical Evolution procedure.\label{fig:ruleGrammar}}
...
\end{figure}
\begin{figure}[H]
\caption{The BNG grammar for the procedure of creating artificial neural networks
with Grammatical Evolution.\label{fig:grammarNNC}}

\begin{lyxcode}
S:=\textless sexpr\textgreater ~~~~~~~~~~~~~~~~~~~~~~~~~~(0)

\textless sexpr\textgreater ::=\textless Node\textgreater ~~~~~~~~~~~~~~~~~~~~(0)

~~~~~~~~~~~\textbar ~\textless Node\textgreater ~+~\textless sexpr\textgreater ~~~~~~~(1)

\textless Node\textgreater ::=\textless num\textgreater{*}sig(\textless sum\textgreater +\textless num\textgreater )~(0)

\textless sum\textgreater ::=~\textless num\textgreater{*}\textless xlist\textgreater ~~~~~~~~~~~~(0)
...
\textless num\textgreater ::=~(\textless dlist\textgreater .\textless dlist\textgreater )~~~~~~~~(0)
...
\end{lyxcode}
\end{figure}
The notation \textless{} \textgreater{} is used to enclose non - terminal
symbols.\textbf{ }The numbers at the end of the production rules represent
the sequence number of each rule. The parameter $d$ is the dimension
of the used dataset ( number of features). Grammatical Evolution produces
valid expressions by starting from the starting symbol $S$ and by
following the production rules. The method selects the following production
rules according to the scheme:
\begin{itemize}
\item The next element denoted as $V$ from the chromosome to be processed
is retrieved
\item The next production rule is selected according to the formula:\textbf{
}
\begin{equation}
...
the following possible expression could be created for the case of
the rule method:
\[
\mbox{if\ensuremath{\left(x_{1}>2+\sin\left(x_{3}\right)\right)}}\mbox{value}=1+\cos\left(x_{2}\right)\ \mbox{else}\ \mbox{value}=1+x_{1}
\]
The terminal symbol value is used to denote the final outcome of the
rule method. For the same case the following constructed neural network
could be created:
\[
\mbox{NNC}(x)=2.45\mbox{sig}\left(1.9x_{1}+3.11x_{3}+2.5\right)+5.9\mbox{sig}\left(10.8x_{2}+6.25\right)
\]
This grammar of Figure \ref{fig:grammarNNC} is able to produce artificial
neural networks using this form:

\begin{equation}
N\left(\overrightarrow{x},\overrightarrow{p}\right)=\sum_{i=1}^{H}p_{(n+2)i-(n+1)}\sigma\left(\sum_{j=1}^{n}x_{j}p_{(n+2)i-(n+1)+j}+p_{(n+2)i}\right)\label{eq:nn}
\end{equation}
The vector $\overrightarrow{x}$ indicates the input vector and the
vector $\overrightarrow{p}$ stands for the parameter set of the neural
network. The integer constant $H$ denotes the number of weights or
processing units. These networks have one processing level, but the
grammar could be easily extended to produce neural networks of additional
levels. The function $\mbox{sig}(x)$ used in the previous example
...
\item \textbf{Denote} with $N_{g}$ the generations allowed and with $N_{c}$
the amount of chromosomes in the genetic population.
\item \textbf{Denote }with $p_{s}$ the used selection rate $\left(p_{s}\le1\right)$
and with $p_{m}$ the used mutation rate $\left(p_{m}\le1\right)$.
\item \textbf{Set} as $p_{cr}$ the rate for the application of the new
crossover operator, where $p_{cr}\le$1
\item \textbf{Set} as $N_{cr}$ the amount of chromosomes the will be chosen
for each chromosome where the new crossover operator will be applied.
\end{enumerate}
\item \textbf{Fitness step}.\label{enu:Fitness-step.}
\begin{enumerate}
\item \textbf{For} $i=1,\ldots,N_{c}$ \textbf{do}
\begin{enumerate}
\item \textbf{Set} as $f_{i}$ the corresponding fitness value of chromosome
$i$. For the case of rule creation model the grammar of Figure \ref{fig:ruleGrammar}
...
\begin{enumerate}
\item \textbf{Apply} the selection procedure: In the first phase, a sorting
is performed for the members of the genetic population according to
the fitness of each chromosome. The $\left(1-p_{s}\right)\times N_{c}$
...
and mutation.
\item \textbf{Execute} the crossover procedure: During this procedure $p_{s}\times N_{c}$
offsprings are produced from the population under processing. For
every set $\left(\tilde{z},\tilde{w}\right)$ of produced children,
two distinct chromosomes $(z,w)$ are chosen from the current population
with tournament selection. The offsprings $\left(\tilde{z},\tilde{w}\right)$
are formulated using the one - point crossover procedure, graphically
outlined in Figure \ref{fig:onePoint}.
\item \textbf{Perform} the mutation procedure. During this procedure a random
number $r\in[0,1]$ is selected for each element of every chromosome
and this element is changed randomly if $r\le p_{m}$.
\item \textbf{Apply} the new crossover operator: For each chromosome $g_{i},\ i=1,\ldots,N_{c}$
a random number $r\in[0,1]$ is selected. If $r\le p_{cr}$ then execute
...
\item \textbf{If} $k\le N_{g}$ goto Step \ref{enu:Fitness-step.}, \textbf{else}
terminate.
\end{enumerate}
\end{enumerate}
%
...
\par\end{centering}
\caption{A working example for the production of children using the method
of one - point crossover.\label{fig:onePoint}}
\end{figure}
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.7]{ga}
\par\end{centering}
\caption{The main steps of the incorporated genetic algorithm are presented
as a flowchart.\label{fig:mainGa}}
...
The new crossover operator executes the one - point crossover method
on a selected chromosome using a set chromosomes that was selected
randomly from the current population. The steps of the proposed operator
are:
\begin{enumerate}
...
The suggested genetic operator was tested on a set of classification
and regression datasets obtained from the recent bibliography and
relevant websites. The internet sources for these datasets are the
following websites:
\begin{enumerate}
\item The UCI dataset repository, \url{https://archive.ics.uci.edu/ml/index.php}(accessed
on 10 October 2024)\citep{UCL}
\item The Keel repository, \url{https://sci2s.ugr.es/keel/datasets.php}(accessed
on 10 October 2024)\citep{Keel}.
\item The Statlib URL \url{http://lib.stat.cmu.edu/datasets/ }(accessed
on 10 October 2024). 
\end{enumerate}

\subsection{The used classification datasets }

A wide series of datasets presenting classification problems is used
here:
\begin{enumerate}
\item \textbf{Appendictis} proposed in \citep{appendicitis}. 
\item \textbf{Australian} dataset proposed in \citep{australian}.
\item \textbf{Balance} dataset \citep{balance}, used in a series of psychological
experiments.
\item \textbf{Circular} dataset, that is a dataset produced artificially.
\item \textbf{Cleveland} dataset \citep{cleveland1,cleveland2}.
\item \textbf{Dermatology} dataset \citep{dermatology}, related to dermatological
deceases. 
\item \textbf{Ecoli} dataset, related to protein problems\citep{ecoli}.
\item \textbf{Fert }dataset, for the detection of possible relations between
fertility and sperm concentration.
\item \textbf{Haberman} dataset, used for breast cancer detection.
\item \textbf{Hayes roth} dataset \citep{hayesroth}. 
\item \textbf{Heart} dataset \citep{heart}, proposed for the detection
of heart diseases.
\item \textbf{HeartAttack} dataset, a medical dataset related to heart diseases. 
\item \textbf{HouseVotes} dataset \citep{housevotes}, that contains the
votes in the U.S. House of Representatives for various cases. 
...
\item \textbf{Mammographic} dataset \citep{mammographic}, used in breast
cancer detection.
\item \textbf{Parkinsons} dataset, proposed in\citep{parkinsons}.
\item \textbf{Pima} dataset \citep{pima}, used in the detection of diabetes.
\item \textbf{Popfailures} dataset \citep{popfailures}, that to do with
climate related measurements.
\item \textbf{Regions2} dataset, proposed in \citep{regions}. 
\item \textbf{Saheart} dataset \citep{saheart}, proposed to detect heart
diseases.
\item \textbf{Segment} dataset \citep{segment}, which contains information
regarding image processing.
\item \textbf{Spiral} dataset, that is a dataset created artificially.
\item \textbf{Student} dataset \citep{student}, a dataset for measurements
in schools. 
\item \textbf{Wdbc} dataset \citep{wdbc}, used in cancer detection.
\item \textbf{Wine} dataset, which contains information about wines. \citep{wine1,wine2}.
\item \textbf{Eeg} datasets, a medical dataset contains measurements from
various experiments regarding EEG \citep{eeg}. The following sub
- datasets were extracted from this dataset: Z\_F\_S, Z\_O\_N\_F\_S,
ZO\_NF\_S and ZONF\_S.
\item \textbf{Zoo} dataset \citep{zoo}, proposed to estimate the category
of some animals.
\end{enumerate}

\subsection{The used regression datasets }

A series of datasets from this category was utilized in the current
work:
\begin{enumerate}
\item \textbf{Abalone} dataset \citep{abalone}.
\item \textbf{Airfoil }dataset, provided by NASA \citep{airfoil}.
\item \textbf{BK} dataset \citep{Stat}, related to the prediction of points
in a basketball game. 
\item \textbf{BL} dataset, that is related to calculations from electricity
experiments.
\item \textbf{Baseball} dataset, used to estimate the income of baseball
players. 
\item \textbf{Concrete} dataset \citep{concrete}, is a dataset related
to the durability of cements in public works. 
\item \textbf{Dee} dataset, related to the prices of electricity.
...
\item \textbf{Housing} dataset \citep{housing}.
\item \textbf{Laser} dataset, that contains measurements from laser experiments.
\item \textbf{LW} dataset, that contains measurements regarding the weight
of babies.
\item \textbf{MORTGAGE}, that contains economic measurements from USA.
\item \textbf{MUNDIAL, }used from the STALIB repository.
\item \textbf{PL} dataset, used from the STALIB repository.
\item \textbf{QUAKE} dataset, used in measurements from earthquakes.
\item \textbf{REALESTATE, }from the STALIB repository.
\item \textbf{SN} dataset, that is used in an experiment related to trellising
and pruning. 
\item \textbf{Treasury} dataset, that is a dataset regarding the economy
of USA.
\item \textbf{TZ} dataset, founded in the STALIB repository.
\item \textbf{VE} dataset, from the STALIB repository.
\end{enumerate}

\subsection{Experimental results}

ANSI C++ was used to code the software for the conducted experiments
using also the Optimus optimization environment, available from \url{https://github.com/itsoulos/GlobalOptimus/}(
accessed on 10 October 2024 ). The experiments were executed 30 times
for each case and the random number generator was initialized using
different seed in every execution. For the case of classification
problems, the average classification error as measured on the test
set was recorded and, for the case of regression datasets, the average
regression error as measured on the test set was recorded. The validation
of experiments was carried out using the method of ten - fold cross
validation.\textbf{ }Table \ref{tab:values} depicts the experimental
settings for the performed experiments.

\begin{table}[H]
\caption{This table presents the values used for the experimental parameters,
used in the conducted experiments.\label{tab:values}}
...
\hline 
\hline 
$N_{c}$ & Chromosomes used & 500\tabularnewline
\hline 
...
\hline 
$H$ & Weights for the neural network & 10\tabularnewline
\hline 
\end{tabular}
\end{table}
Table \ref{tab:experClass} outlines the results for the performed
experiments on the classification datasets while Table \ref{tab:experRegression}
holds the results for the mentioned regression datasets. The following
notations are used in the experimental tables:
\begin{enumerate}
\item The column BFGS represents the incorporation of the BFGS method \citep{powell}
to train an artificial neural network with $H$ hidden nodes.
\item The column GEN outlines the experimental results using a genetic algorithm
\citep{doublepop} to train an artificial neural network with $H$
hidden nodes. The values for the critical parameters of the genetic
algorithm are included in Table \ref{tab:values}. 
\item The column RULE refers to the simple rule construction method \citep{rule},
without the incorporation of the new crossover operator.
\item The column NNC refers to the method of neural network construction
\citep{nnc_tsoulos}. This method was applied without the new operator.
...
\item The column NNC\_CROSS depicts the results for the neural construction
method with the assistance of the new genetic operator.
\item The average error is depicted in row under the name AVERAGE.
\end{enumerate}
\begin{table}[H]
\caption{This table shows the average classification error as calculated on
the corresponding test set for the classification datasets using all
machine learning models. \label{tab:experClass}}
...
\end{table}
\begin{table}[H]
\caption{This table shows the experimental results when the regression datasets
were used and the numbers stand for the average regression error as
measured on the corresponding test set.\label{tab:experRegression}}
...
is a significant reduction in the mean error using the new genetic
operator in both machine learning models. In a wide range of data
sets, the proposed technique drastically reduces the error of data
classification or fitting, as it is also represented in the graphs
\ref{fig:ruleDatasets} and \ref{fig:nncDatasets}. These graphs show
the number of datasets in which the application of the new genetic
operator resulted in a drastic reduction in the corresponding error. 
...
\par\end{centering}
\textbf{\caption{Box plot used to compare the original rule construction model and
the improved one that utilizes the new crossover operator.\label{fig:boxNncClass}}
}

\end{figure}
Box plots for the same comparisons as deduced from the results using
the series of regression datasets are shown in Figures \ref{fig:boxRuleRegression}
...
\par\end{centering}
\textbf{\caption{Box plot used to compare the original rule construction model and
the improved one that utilizes the new crossover operator for the
regression datasets.\label{fig:boxRuleRegression}}
}
...
\textbf{\caption{Box plot used to compare the original rule construction model and
the improved one that utilizes the new crossover operator for the
regression datasets.\label{fig:boxNncRegression}}
}
...
Also, a statistical comparison was performed using the two machine
learning methods and the enhanced ones that use the new crossover
operator. This comparison was performed using the classification datasets,
and it is shown in Figure \ref{fig:statClass}.

\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.4]{statistical_class}
\par\end{centering}
\caption{Statistical test for all the improved machine learning methods and
the original methods. The methods were tested on the classification
datasets.\label{fig:statClass}}
\end{figure}
Moreover, an additional test was executed in order to estimate the
...
applied on the classification datasets using a series of values for
$p_{cr}$ and the results are shown in Table \ref{tab:pcrExper}.
\begin{table}[H]
\caption{The effect of a series of values of $p_{cr}$ to the RULE model with
application on the classification datasets.\label{tab:pcrExper}}
...
Another experiment was performed in order to estimate the importance
...
neural network construction method was tested on the classification
datasets using a series of values for the parameter $N_{cr}$ while
the parameter $p_{cr}$ was fixed to 2.5\%. The outcomes obtained
from this experiment are depicted in Table \ref{tab:experNCR}.

\begin{table}[H]
\caption{The effects of the parameter $N_{cr}$ to the NNC machine learning
model. The classification datasets were used in this experiment. In
all experiments the value $p_{cr}$ was set to 0.025. \label{tab:experNCR}}
...
in Figure \ref{fig:timeNNC}, where the computation time for the neural
network construction method is plotted with respect to the $N_{cr}$
.
\begin{figure}[H]
\begin{centering}
...
\end{figure}
The average computation time increases dramatically as the critical
...
This dramatic increase in required execution time can be reduced by
incorporating modern parallel libraries. These programming techniques
may include the application of the Message Passing Interface (MPI)
library \citep{mpi} or the incorporation of the OpenMP library \citep{openmp}.
...
\begin{itemize}
\item A rule construction method, that constructs rules in language similar
to the C programming language for data classification or regression
problems.
\item A method that constructs artificial neural networks.
\end{itemize}
Several datasets from the literature were used to test the proposed
method. In the vast majority of cases, the application of the new
genetic operator resulted in a drastic reduction of the corresponding
classification or data fitting error. Furthermore, to measure to asses
the sensitivity of the proposed method to some critical parameters,
more experiments were conducted in which these parameters were changed
inside a wide range of values. Boosting these values improves the
performance of machine learning methods by applying the new genetic
operator, but up to a point. In addition, an increase in the chromosomes
involved in the genetic operator, has a significant increase in the
required execution time, as was also seen in the performed experiments.
However, with the use of recent techniques that can utilize modern
parallel computing structures, this additional time can be significantly
reduced.

Improvements of the proposed operator in future studies may include
the application of the new crossover in other machine learning methods
based on Grammatical Evolution, a parallel implementation of the operator
or even the usage of this operator in other tasks involving Genetic
Algorithms.
