...
dat <- read_excel("C:/Users/admin/Desktop/LARGE_SCALE/t2.xlsx")
df <- dat |>
  rename(FUNCTION = 1) |>
...
cmp <- list(
  ...
  c("RANDOM(R)", "MIGRANT(T)"),
  c("RANDOM(T)", "MIGRANT(R)"),
  c("RANDOM(T)", "MIGRANT(T)"),
  c("MIGRANT(R)", "MIGRANT(T)")
  )
...
ggboxplot(df, x = "Method", y = "Error", color = "Method", shape = "Method", add = "jitter",
  ...
  stat_compare_means(paired = TRUE, method = "t.test", comparisons = cmp, label = "p.signif",
                     label.y = c(30000, 35000, 40000, 45000, 50000, 55000)) +
