...
dat <- read_excel("/Users/glykeriakyrou/Desktop/t4.xlsx")
df <- dat |>
  rename(FUNCTION = 1) |>
...
cmp <- list(
  c("RANDOM(0.005)", "RANDOM(0.01)"),
  c("RANDOM(0.005)", "RANDOM(0.02)"),
  c("RANDOM(0.005)", "RANDOM(0.04)"),
  c("RANDOM(0.01)", "RANDOM(0.02)"),
  c("RANDOM(0.01)", "RANDOM(0.04)"),
  c("RANDOM(0.02)", "RANDOM(0.04)")
  )
...
ggboxplot(df, x = "Method", y = "Error", color = "Method", shape = "Method", add = "jitter",
  ...
  stat_compare_means(paired = TRUE, method = "t.test", comparisons = cmp, label = "p.signif",
                     label.y = c(15000, 17000, 19000, 21000, 23000, 25000)) +
  stat_compare_means(paired = TRUE, var.equal = FALSE, label.y = 37000, label.x = 1) +
