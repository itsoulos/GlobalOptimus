...
dat <- read_excel("C:/Users/admin/Desktop/LARGE_SCALE/t3.xlsx")
df <- dat |>
  rename(FUNCTION = 1) |>
...
cmp <- list(
  c("RANDOM(U)", "RANDOM(K)"),
  c("RANDOM(U)", "MIGRANT(U)"),
  c("RANDOM(U)", "MIGRANT(K)"),
  c("RANDOM(K)", "MIGRANT(U)"),
  c("RANDOM(K)", "MIGRANT(K)"),
  c("MIGRANT(U)", "MIGRANT(K)")
  )
...
ggboxplot(df, x = "Method", y = "Error", color = "Method", shape = "Method", add = "jitter",
  ...
  stat_compare_means(paired = TRUE, method = "t.test", comparisons = cmp, label = "p.signif",
                     label.y = c(15000, 19000, 23000, 27000, 31000, 35000)) +
  stat_compare_means(paired = TRUE, var.equal = FALSE, label.y = 47000, label.x = 1) +
