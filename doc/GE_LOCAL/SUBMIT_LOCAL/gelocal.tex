...
methods should be adopted. The current work utilizes a modified version
...
deep learning \citep{siman_deep} etc. In this paper, the modified
Simulated Annealing method was repeatedly applied to randomly chosen
chromosomes of Grammatical Evolution for the problem of constructing
artificial neural networks. The proposed method was tested on a wide
series of classification and regression problems from the relevant
literature and the results seem to be promising.
...
modification is outlined in detail, in section \ref{sec:Results}
the datasets used in the experiments are listed and the experimental
results are presented and finally in section \ref{sec:Conclusions}
...
The chromosomes of Grammatical Evolution represent production rules
of the provided BNF grammar. These grammars are usually defined as
a set \textbf{$G=\left(N,T,S,P\right)$}, where
\begin{itemize}
...
\end{equation}
where $H$ stands the number of processing units of the neural network
and $d$ is the dimension of vector $\overrightarrow{x}$ and the
vector $\overrightarrow{w}$ defines the weight vector of the neural
network. The function sig(x) represents the sigmoid function $\sigma(x)$
defined as: 
\begin{equation}
...
denotes the dimension of the input data. The production of valid expressions
initiates from the symbol $S$ and using a series of steps, creates
a program by replacing non - terminal symbols with the right hand
of the selected production rule. The Grammatical Evolution selects
production rules using the following scheme:
\begin{itemize}
\item Get the next element V from the given chromosome.
\item Select the next rule using the equation Rule = V mod NR, where NR
is the number of production rules for the current non -- terminal
symbol. 
\end{itemize}
\begin{figure}
...
\item \textbf{Define} as $L_{c}$ the number of chromosomes that will participate
in local search procedure.
\item \textbf{Initialize} the chromosomes as vectors of random integers. 
...
are selected using tournament selection. The new offsprings are produced
using the one - point crossover procedure, which is graphically shown
...
\begin{enumerate}
\item Obtain the best chromosome and create the associated neural network
$C^{*}$.
\item Evaluate the neural network $C^{*}$ in the test set of the objective
...
\par\end{centering}
\caption{The one - point crossover used in Grammatical Evolution.\label{fig:onePoint}}
...
The current work was applied to a wide series of datasets from different
areas and the experimental results were compared with other machine
learning techniques. These datasets can be downloaded freely from
...
\item \textbf{Baseball} dataset, It was used to predict the income of baseball
players. 
\item \textbf{Concrete} dataset \citep{concrete}, which is a civil engineering
dataset.
\item \textbf{Dee} dataset, related to the price of electricity.
...
used and, with the help of the programming environment Optimus, that
can be downloaded freely from \url{https://github.com/itsoulos/OPTIMUS/}(
accessed on 20 March 2024 ). The experiments were conducted 30 times
using different initialization for the random seed each time. The
used random generator was the function drand48() of the C programming
language. The experimental results were validated using 10 - fold
cross validation. The average classification error is reported for
the case of classification datasets and the average regression error
for the case of regression datasets. These errors are measured on
the corresponding test set. The values for the experimental parameters
...
\item The column MLP represents the results obtained by the training of
an artificial neural network with $H=10$ processing nodes. The neural
network was trained using a genetic algorithm and the BFGS local optimization
method \citep{powell}. 
\item The column RBF stands for the results obtained by an RBF network with
$H=10$ processing nodes.
\item The column NNC denotes the application of the original Neural Construction
technique performed with the assistance of Grammatical Evolution.
\item The column NNC-S denotes the application of the proposed local optimization
...
values for the critical parameter $g$ of the proposed Simulated Annealing
variant. This parameter is used to control the number of random changes
performed on any given chromosome. The experimental results in the
classification datasets are shown in Table \ref{tab:g_experiments}.
...
Furthermore, an additional experiment was performed by varying the
parameter $R$ of the proposed Simulated Annealing variant. This parameter
controls the range of changes performed in any given chromosome. The
results for this experiment are shown in Table \ref{tab:r_exper}.
And this time the method appeared quite robust in its performance,
without large variations in the error as measured in the test set. 
...
In this extension, an application of a modified optimization algorithm
...
Simulated Annealing method, that was applied on randomly selected
chromosomes of the Grammatical Evolution procedure. This method was
chosen because of its widespread use in many applications but because
of its ability to handle integer representations, since the chromosomes
in Grammatical Evolution are vectors of integers. Of course, other
local optimization techniques could be incorporated, such as tabu
search \citep{tabu} or hill climbing \citep{hill} as feature extensions
of the proposed method.

The proposed modification was applied to the Neural Network Construction
method and its efficiency was measured on a wide series of classification
and regression datasets from the relevant literature. Based on the
experimental results, it has become clear that the proposed variant
significantly improves the performance of the technique both on classification
datasets and on data fitting datasets. Also, the effectiveness and
the robustness of the proposed modification were measured using experiments
with different values of some critical parameters of the current Simulated
Annealing variant. These experiments indicated that the method does
not depend heavily on the selection of any critical parameter of the
